...
class DetailPage extends CMSUpdatePageBase
{
    ...
    public function __construct($arg)
    {
        ...
        if (!UserInfoUtil::isDefaultUser() || strlen($adminID) < 1) {
            $adminID = UserInfoUtil::getUserId();
        }
        if (null===$adminID) {
        ...
        "html" => $this->buildCustomField($admin->getId())
        ));
        ...
        //ページの末尾にカスタムHTMLを追加
        if (file_exists(SOY2::RootDir() . "config/administrator.detail.custom.php")) {
            require_once(SOY2::RootDir() . "config/administrator.detail.custom.php");
        }
