...
class EditHtaccessPage extends CMSUpdatePageBase
{
    ...
    public function __construct($args)
    {
        ...
        if (!is_writable($site->getPath() . F_HTACCESS)) {
        ...
        $this->showMessage();
        $this->showBackupFiles($site->getPath() . F_HTACCESS);
    }

    private function buildForm($site)
    {
        $filepath = $site->getPath() . F_HTACCESS;
    ...
    {
        $site = $this->getSite();
        return file_put_contents($site->getPath() . F_HTACCESS, $contents);
    }
