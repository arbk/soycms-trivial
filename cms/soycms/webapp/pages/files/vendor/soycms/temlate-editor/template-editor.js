...
//document.cookie = 'editor_mode=' + editor_mode + '; expires=' + new Date(2030, 1).toUTCString();
  document.cookie = 'editor_mode=' + editor_mode + '; expires=' + movedate(new Date,30,0,0,0,0,0).toUTCString() + '; path=' + soycms.env.script_dir + '; samesite=Lax;';
...
function saveCSS(){
  ...
  $.ajax({
    type: "post",
    url: cssURL+"/SaveCSS",
    data: $("#css_editor").serialize()+'&'+$("#css_list").serialize() + "&s_token=" + $("#main_form").find("[name=s_token]").val(),
    dataType: "json",
    success: function(json){
      alert(json.result);
      $("#main_form").find("[name=s_token]").val(json.s_token);
    },
    error: function(json){
...
function save_template_ajax(url,trials,loading,ele){
  ...
  if(trials>0){
    $.ajax({
      url: url,
      type : "post",
      data : "template=" + encodeURIComponent(content) + "&s_token=" + $("#main_form").children('input[name=s_token]').val(),
      success : function(data){

          var res = eval("array="+data);

          if($("#main_form")){
            $("#main_form").children('input[name=s_token]').val(res.s_token);
          }
          if(res.text.match(/^0$/)){
            trials--;
            //s_tokenが古い場合に備えて何回かやり直す
