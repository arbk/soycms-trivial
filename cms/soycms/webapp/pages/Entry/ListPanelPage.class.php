...
class ListPanelPage extends CMSUpdatePageBase
{
    public function doPost()
    {
        ...
        if ($result->success()) {
            echo '<!DOCTYPE html><html><head><title>-</title><script lang="text/JavaScript">window.parent.location.reload();</script></head></html>';
        }
        exit;
    }
...
class LabelList extends HTMLList
{
    ...
    protected function populateItem($entity)
    {
        ...
        $this->createAdd("label_check", "HTMLCheckBox", array(
            "name"      => "label[]",
            "value"     => $entity->getId(),
            "selected"=>is_array($this->selectedIds)&&in_array($entity->getId(), $this->selectedIds, true),
            "elementId" => $elementID,
        ));
