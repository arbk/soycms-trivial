...
class InsertBlogLinkPage extends CMSWebPageBase
{
    ...
    public function __construct($arg)
    {
        ...
        //表示しないものは選択肢に出さない
        if (!$obj->getGenerateTopFlag()) {
            DisplayPlugin::hide("show_top");
        }
        if (!$obj->getGenerateEntryFlag() || !count($entries)) {
            DisplayPlugin::hide("show_entry");
        }
        if (!$obj->getGenerateCategoryFlag() || !count($labels) || !count($entries)) {
            DisplayPlugin::hide("show_category");
        }
        if (!$obj->getGenerateMonthFlag() || !count($entries)) {
            DisplayPlugin::hide("show_archive");
        }
        if (!$obj->getGenerateRssFlag()) {
            DisplayPlugin::hide("show_feed");
        }
    ...
    {
        return $this->run("Blog.EntryListAction", array(
            "pageId"=>$this->pageId,
