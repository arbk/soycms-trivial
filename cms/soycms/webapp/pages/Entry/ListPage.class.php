...
class ListPage extends CMSUpdatePageBase
{
     ...
     * クッキーに保存
     */
    public function updateCookie($labelIds)
    {
        $timeout = 0;
...
//          setcookie("Entry_List_Limit", $value, $timeout, $path);
    ...
    public function __construct($arg)
    {
        ...
        //include_once(__DIR__.'/_EntryBlankPage.class.php');
...
//      ));
        ...
        $this->addModel("create_link_box", array(
            "visible"=>isset($labelIds[0]) && is_numeric($labelIds[0])
        ));
   ...
   */
    private function getEntries($offset, $limit, $labelIds)
    {
        if (strpos($_SERVER["PATH_INFO"], "/Closed")) {
        ...
        } elseif (strpos($_SERVER["PATH_INFO"], "/OutOfDate")) {
            ...
            return array($entities,$totalCount,min($offset, $totalCount));
        } elseif (strpos($_SERVER["PATH_INFO"], "/NoLabel")) {
