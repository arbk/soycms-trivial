...
class EntryPage extends CMSEntryEditorPageBase
{
    ...
    public function __construct($arg)
    {
        ...
        if ((null===$this->detail->getBlogLabelId())) {
...
//      $toolBoxForSelectTemplte .= '<span class="input-group-addon">雛形を読み込む<span soy:message="HELP_ENTRY_TEMPLATE"></span></span>';
        ...
        foreach ($this->getEntryTemplateList() as $templateId => $templateObj) {
            $toolBoxForSelectTemplte .= '<option value="'.soy2_h($templateId).'">'.soy2_h($templateObj->getName()).'</option>';
        }
...
//      $this->setupWYSIWYG($this->id, $this->detail->getBlogLabelId(), false);
    ...
    //             return array( "id "=> $v->getId(),"filePath" => $v->getFilePath());
    //         }, $list);
    ...
    public function setupForm()
    {
        ...
        // この記事のトラックバックURI
        $this->createAdd("trackback_uri", "HTMLInput", array(
            "value"=>UserInfoUtil::getSiteUrl() . $this->detail->getEntryPageURL(true) . $entry->getId() . "?trackback",
            "name"=>"trackback_uri"
        ));
