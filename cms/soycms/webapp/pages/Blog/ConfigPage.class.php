...
class ConfigPage extends CMSWebPageBase
{
    ...
    public function __construct($args)
    {
        ...
        // ラベルが無いときのメッセージ
        require_once(__DIR__ . '/_LabelBlankPage.class.php');
...
//      $this->setupWYSIWYG();
      ...
      // 公開期間フォームの表示
        ...
        $sortTypeOptions = array(
            BlogPage::ORDER_TYPE_CDT=>"作成日",
            BlogPage::ORDER_TYPE_TTL=>"タイトル"
        );
        $sortOptions = array(
            BlogPage::ENTRY_SORT_DESC=>"降順",
            BlogPage::ENTRY_SORT_ASC=>"昇順"
        );

        $form->addSelect("top_entry_sort_type", array(
            "name"=>"topEntrySortType",
            "options"=>$sortTypeOptions,
            "selected"=>$page->getTopEntrySortType()
        ));
        ...
        $form->addSelect("month_entry_sort_type", array(
            "name"=>"monthEntrySortType",
            "options"=>$sortTypeOptions,
            "selected"=>$page->getMonthEntrySortType()
        ));
        ...
        $form->addSelect("category_entry_sort_type", array(
            "name"=>"categoryEntrySortType",
            "options"=>$sortTypeOptions,
            "selected"=>$page->getCategoryEntrySortType()
        ));
        ...
        // 出力項目
        $opdataOptions = array(
            BlogPage::ENTRY_OPDATA_ALL=>"全て",
            BlogPage::ENTRY_OPDATA_CNT=>"本文",
            BlogPage::ENTRY_OPDATA_TTL=>"タイトル"
        );
        $form->addSelect("top_entry_opdata", array(
            "name"=>"topEntryOpdata",
            "options"=>$opdataOptions,
            "selected"=>$page->getTopEntryOpdata()
        ));
        $form->addSelect("month_entry_opdata", array(
            "name"=>"monthEntryOpdata",
            "options"=>$opdataOptions,
            "selected"=>$page->getMonthEntryOpdata()
        ));
        $form->addSelect("category_entry_opdata", array(
            "name"=>"categoryEntryOpdata",
            "options"=>$opdataOptions,
            "selected"=>$page->getCategoryEntryOpdata()
        ));
...
//      }
        ...
        if ($page->isActive() == Page::PAGE_ACTIVE) {
            ...
            CMSToolBox::addLink(
                CMSMessageManager::get("SOYCMS_SHOW_BLOGPAGE"),
                $pageUrl,
                false,
                "this.target = '_blank'"
            );
            CMSToolBox::addLink(
                CMSMessageManager::get("SOYCMS_SHOW_MONTHLYARCHIVEPAGE"),
            ...
            CMSToolBox::addLink(
                CMSMessageManager::get("SOYCMS_SHOW_RSS"),
                $pageUrl.$page->getRssPageUri()."?feed=rss",
                false,
                "this.target = '_blank'"
            );
            CMSToolBox::addLink(
                CMSMessageManager::get("SOYCMS_SHOW_ATOM"),
