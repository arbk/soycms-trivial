...
class TemplateList extends HTMLList
{
    ...
    protected function populateItem($entity, $key)
    {
        ...
        $html = array();
        $html[] = '<a class="btn btn-default btn-sm" href="'.soy2_h(SOY2PageController::createLink("Template.Create").'?edit&id='.$key).'">'.soy2_h(CMSMessageManager::get("SOYCMS_EDIT")).'</a>';

        if ($this->template->getPageType() != Page::PAGE_TYPE_BLOG) {
            $html[] = '<a class="btn btn-default btn-sm" href="'.soy2_h(SOY2PageController::createLink("Template.Create").'?delete&id='.$key).'">'.soy2_h(CMSMessageManager::get("SOYCMS_DELETE")).'</a>';
        }
