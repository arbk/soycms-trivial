...
class SOYCMSOutputContents
{
    ...
    //キャッシュ保存ディレクトリ
    const CACHEDIR = "/".SOYCMS_CACHE_DIRNAME."/soy_static/";
    private $cacheDir;
...
//      $cache_inc = "{$this->cacheDir}{$this->md5_pathinfo[0]}/{$this->md5_pathinfo[1]}/{$this->md5_pathinfo}.inc.php";
     ...
     * 通常の出力処理
     */
    public static function execute_normal()
    {
//      require_once(_CMS_COMMON_DIR_."/site.inc.php");
        SOY2DAOConfig::Dsn(_SITE_DSN_);
        if (defined("_SITE_DB_USER_")) {
            SOY2DAOConfig::user(_SITE_DB_USER_);
        }
        if (defined("_SITE_DB_PASSWORD_")) {
            SOY2DAOConfig::pass(_SITE_DB_PASSWORD_);
        }
        $CMSPageController = SOY2PageController::init("CMSPageController");
        try {
            SOY2PageController::run();
        } catch (Exception $e) {
            error_log("Running PageController is failed. : ".__METHOD__."\n".$e->getMessage()."\n".$e->getQuery());
        }
    ...
    public function execute()
    {
        ...
        if ($this->hasValidCache()) {
            ...
            //キャッシュファイルを示すヘッダー
            if (self::DEBUG_MODE >= self::DEBUG_MODE_HEADER) {
                header("X-SOYCMS-Cache: ".$this->md5_pathinfo);
            }
            ...
            if ($this->ifNotModified() && !headers_sent()) {
                ...
                //出力
                if (file_exists($this->cache_header)) {
                    include($this->cache_header);
                }
                $this->output();
            }
        } else {
...
//              || !file_exists(dirname($this->cache)) || !is_writable(dirname($this->cache))
            ...
            //キャッシュディレクトリを作成
            $this->makeDir();
            if (!file_exists(dirname($this->cache)) || !is_writable(dirname($this->cache))) {
                //キャッシュ保存先に書き込めない
                error_log("Cache dir is not exist or is not writable. : ".__METHOD__);
                return;
            }
...
//          }
    ...
    private function hasValidCache()
    {
        return
            ...
            //キャッシュの有効期間内
            && (  0 >= SOYCMS_CACHE_LIFETIME || $this->request_time < filemtime($this->cache) + SOYCMS_CACHE_LIFETIME )
            //データベースに変更がない
    ...
    {
        if (!ob_start("ob_gzhandler")) { //ブラウザが受け入れてないならfalseが返る
            ob_start();
        }
        if (null===$contents) {
            readfile($this->cache);
            if (self::DEBUG_MODE >= self::DEBUG_MODE_HTML && isset($_SERVER["REQUEST_URI"])) {
                echo '<div class="cache" style="background-color:orange;position:absolute;top:0px;left:0px;width:100%;background-color:orange;line-height:2em;height:2em;text-align:center;">This is cache. '.$_SERVER["REQUEST_URI"].' => '.$this->md5_pathinfo.'</div>';
            }
        } else {
            if (strlen($this->generate_time)) {
                header("Last-Modified: ".gmdate("D, d M Y H:i:s T", $this->generate_time), true);
            }
            echo $contents;
            if (self::DEBUG_MODE >= self::DEBUG_MODE_HTML) {
                echo '<div style="position:absolute;top:0px;left:0px;width:100%;background-color:lightblue;line-height:2em;height:2em;text-align:center;">This is NOT cache.</div>';
            }
        }

        header("Content-Length: " . ob_get_length(), true);
        soycms_site_header_output();
    ...
    {
        if (!file_exists(dirname($this->cache))) {
            mkdir(dirname($this->cache), F_MODE_DIR, true);
        }
    }
    ...
    private function saveHeaders($CMSPageController)
    {
        ...
        if ($CMSPageController->getPageType() == Page::PAGE_TYPE_ERROR
        ...
        if (count($h) >0) {
            ...
            chmod($this->cache_header, F_MODE_FILE);
        }
    }
    ...
    private function saveLifetime($lifetime)
    {
        ...
        chmod($this->lifetimeFile, F_MODE_FILE);
        touch($this->lifetimeFile, $lifetime);
        if (file_exists($this->lifetimeFile) && filemtime($this->lifetimeFile) != $lifetime) {
            unlink($this->lifetimeFile);
        }
    }
