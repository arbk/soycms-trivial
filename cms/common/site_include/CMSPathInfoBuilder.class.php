...
class CMSPathInfoBuilder extends SOY2_PathInfoPathBuilder
{
    ...
    public static function parsePath($path)
    {
        ...
        if (!count($candidateList) && count($_uri) === 1) {
            return array($_uri[0], $args);
        }
        ...
        $onLoads = CMSPlugin::getEvent('onPathInfoBuilder');
    ...
    private static function _getCandidatePageList($uri)
    {
        ...
        if (!count($res)) {
            $res = $dao->executeQuery("SELECT uri FROM Page WHERE uri NOT LIKE :uri", array(":uri" => "index%"));
        }
