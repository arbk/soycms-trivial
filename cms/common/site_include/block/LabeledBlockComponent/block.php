    ...
    public function getViewPage($page)
    {
        ...
        if (null!==$this->displayCountFrom && is_numeric($this->displayCountFrom)) {
            ...
            && ($this->displayCountTo >= $this->displayCountFrom)) {
                $limit = $this->displayCountTo - (int)$this->displayCountFrom + 1; // n件目～m件目はm-n+1個のエントリ
            }
...
//      && null!==$limit && $limit > 0 && null!==$offset
//      ) {
...
//              $categoryPageUrl = $page->siteRoot . $blogPage->getCategoryPageURL(); // TOOD
//              }
            } catch (Exception $e) {
...
//      //
//      if(!strlen($this->pagingParameter) && ($this->displayCountFrom > 1 && (null===$this->displayCountTo))){
...
//      ));
    ...
    public function getInfoPage()
    {
        ...
        try {
            ...
            $res = $label->getCaption() . " " . ((strlen($this->displayCountFrom) or strlen($this->displayCountTo)) ? $this->displayCountFrom . "-" . $this->displayCountTo : "");
        } catch (Exception $e) {
            error_log($e->getMessage());
...
// {
//     public static function buildPager($appComponent, $pageUrl, $count, $total, $offset, $page, $limit, $pagerParamKey)
//     {
//         $start = $offset;
//         $end = $start + $count;
//         if ($end > 0 && $start == 0) {
...
//
//         $query = $_GET;
//         if (isset($query[$pagerParamKey])) {
//             unset($query[$pagerParamKey]);
//         }
