...
//      $entryDAO = SOY2DAOFactory::create("cms.EntryDAO");
        ...
        $root_url = UserInfoUtil::getSiteURLBySiteId("");
        $this->addLabel("host_url", array(
        "text"=>rtrim($root_url, "/"),
        "soy2prefix"=>"cms"
        ));
        $this->addLabel("root_url", array(
          "text"=>$root_url,
          "soy2prefix"=>"cms"
        ));
        ...
        $this->addLabel("abs_page_url", array(
          "text"=>$this->siteUrl.$this->page->getUri(),
          "soy2prefix"=>"cms"
        ));
      ...
      // メッセージの設定
        $this->addMessageProperty("host_url", '<?php echo $' . $this->_soy2_pageParam . '["host_url"]; ?>');
        $this->addMessageProperty("root_url", '<?php echo $' . $this->_soy2_pageParam . '["root_url"]; ?>');
        $this->addMessageProperty("site_name", '<?php echo $' . $this->_soy2_pageParam . '["site_name"]; ?>');
        $this->addMessageProperty("site_url", '<?php echo $' . $this->_soy2_pageParam . '["site_url"]; ?>');
        ...
        $this->addMessageProperty("page_url", '<?php echo $' . $this->_soy2_pageParam . '["page_url_attribute"]["href"]; ?>');
        $this->addMessageProperty("abs_page_url", '<?php echo $' . $this->_soy2_pageParam . '["abs_page_url"]; ?>');
        $this->addMessageProperty("page_link", '<?php echo $' . $this->_soy2_pageParam . '["page_link"]; ?>');
  ...
  //  }
    ...
    public function executePlugin($id, $soyValue, $plugin)
    {
        while (true) {
     ...
     * @override
     */
    public function parsePlugin($plugin = null)
    {
      // リンクの置換え
     ...
     */
    public function onLoadPageTemplate($html)
    {
    ...
    public function buildModules($mdl_suffix = "module", $immediate = false)
    {
        SOY2::import("site_include.CMSPageModulePlugin");
        $plugin = new CMSPageModulePlugin();
        $plugin->setImmediate($immediate);

        while (true) {
            list($tag, $line, $innerHTML, $outerHTML, $value, $suffix, $skipendtag) =
            $plugin->parse($mdl_suffix, "[a-zA-Z0-9\.\_]+", $this->_soy2_content);
    ...
    public function buildTmplModules()
    {
        $this->buildModules("module_tmpl", true);
    }

    /**
     * コメントを消去します。
     */
    public function parseComment($html)
    {
        return self::deleteComment($html);
    }

    public static function deleteComment($html)
    {
      // コメントがなければ何もせずにそのまま返す
        ...
        $startRegex = '/(<[^>]*[^\/]' . self::SOYCMS_COMMENT_PHRASE . '[^>]*>)/';
        $endRegex = '/(<[^>]*\/' . self::SOYCMS_COMMENT_PHRASE . '[^>]*>)/';
     ...
     */
    public function beforeConvert($html)
    {
        return $html;
    }
     ...
     * 最終的に表示するHTMLがここに設定される
     */
    public function afterConvert($html)
    {
...
//      return SOYCMSEmojiUtil::replace($html,$this->siteConfig->getCharsetText());
     ...
     */
    public function getAbusolutePageUrl($path)
    {
        if ($this->siteConfig->getConfigValue("url")) {
            return $this->siteConfig->getConfigValue("url");
        } else {
