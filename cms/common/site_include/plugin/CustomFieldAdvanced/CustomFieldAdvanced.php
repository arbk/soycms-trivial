...
class CustomFieldPluginAdvanced
{
  ...
  // アドバンス用のフィールドで高度な設定のラベル紐づけを考慮したフィールドリストを毎回取得しないようにするフラグ
    ...
    // 設定
    public $displayTitle = 0; // 「カスタムフィールド」を表示する
    public $displayID = 0;    // IDを表示する
    ...
    public function display($arg)
    {
        ...
        if ($this->acceleration == 1) {
            if (!$this->displayLogic) {
                $this->displayLogic = SOY2Logic::createInstance("site_include.plugin.CustomFieldAdvanced.logic.DisplayLogic");
            }
...
//    $isLabelField = CustomfieldAdvancedUtil::checkIsLabelField($customFields);    // @ToDo メモリをたくさん食うからブログ一覧やラベルブロックでは禁止にしたい
                  ...
                  // 空の時の動作
                    ...
                    if ($master->getType() == "link") {
                      ...
                      // 画像の場合
                        ...
                        $imgProps = $this->_getImgProps("image");
                        if (count($imgProps)) {
    ...
    //        }
                    ...
                    if (strlen($master->getOutput()) > 0) {
                      // リンクタイプ以外でhrefを使う場合
                        if ($master->getOutput() == "href" && $master->getType() != "link") {
                          ...
                          // 下方互換
                        } elseif ($master->getType() == "image" && $master->getOutput() == "src") {
                          // 上で処理をしているため何もしない
              ...
              //          $attr[$extraOutput] = is_array($extraValues) && isset($extraValues[$extraOutput]) ? $extraValues[$extraOutput] : "";
                            $attr[$extraOutput] = (isset($extraValues[$extraOutput]) && 0 < strlen($extraValues[$extraOutput]))
                                ? $extraValues[$extraOutput]
                                : (("image" == $master->getType() && "alt" == $extraOutput) ? " " : "");
                        }
                  ...
                  //ペアフィールド
                    if ($master->getType() == "pair" && strlen($master->getExtraValues())) {
                      ...
                      //後方互換
                        if (isset($extraValues["pair"]) && is_array($extraValues["pair"])) {
                            $extraValues = $extraValues["pair"];
                        }
    ...
    private function _getImgProps($type = "image")
    {
        ...
        foreach ($this->customFields as $fieldId => $field) {
            if ($field->getType() == $type) {
                ...
                if (!count($extraOutputs)) {
                    continue;
                }
                foreach ($extraOutputs as $output) {
                    $output = trim($output);
                    if (!strlen($output) || is_numeric(array_search($output, $this->properties[$type]))) {
                        continue;
                    }
        ...
        foreach ($this->customFields as $key => $field) {
          ...
          // 新規作成の場合
                try {
                    ...
                    error_log("EntryAttribute failed to insert. : " . __METHOD__);
                }
            }
        }
     ...
     * 記事複製時
     */
    public function onEntryCopy($args)
    {
        list($old, $new) = $args;
     ...
     */
    public function onEntryRemove($args)
    {
        $dao = $this->dao;
     ...
     */
    public function updateAdvance($id, $obj)
    {
        if (isset($this->customFields[$id])) {
     ...
     * プラグイン管理画面 表示順の変更
     */
    public function moveField($id, $diff)
    {
        if (isset($this->customFields[$id])) {
     ...
     * プラグイン管理画面
     */
    public function insertField(CustomField $_field)
    {
    ...
    public function getScripts()
    {
        ...
        $script .= file_get_contents(__DIR__ . "/custom_field.js");
        $script .= '</script>';
     ...
     */
    public function getCustomFields($entryId, $labelIdWithBlock = null, $blogCategoryLabelList = array())
    {

        $dao = $this->dao;

        if ((null===$labelIdWithBlock)) {
            $customFields = $this->customFields;
            ...
            if ($labelIdWithBlock != $this->prevLabelId) {
                ...
                foreach ($this->customFields as $customField) {
                  ...
                  // ブログのカテゴリ設定分を確認する。存在している場合はcontinue
                        if (count($blogCategoryLabelList) > 0 && in_array($labelId, $blogCategoryLabelList)) {
                            continue;
                        }
     ...
     */
    public function checkLabelConfigOnBlock($labelId, $labelIdWithBlock)
    {
        return (null!==$labelId && $labelId > 0 && $labelId != $labelIdWithBlock);
    }
    ...
    {
        $csvLogic = SOY2Logic::createInstance("site_include.plugin.CustomFieldAdvanced.logic.ExImportLogic", array(
        "pluginObj"=>$this
        ));
        $csvLogic->importFile();
    }
     ...
     * プラグイン管理画面 設定の保存
     */
    public function updateDisplayConfig($config)
    {
      // 表示設定
        $this->displayTitle = (isset($config["display_title"]) && $config["display_title"] > 0) ? 1 : 0;
        $this->displayID = (isset($config["display_id"]) && $config["display_id"] > 0) ? 1 : 0;
        $this->acceleration = (isset($config["acceleration"]) && $config["acceleration"] > 0) ? 1 : 0;
...
//  }
//
//  $file = file_get_contents(__DIR__ . "/sql/init_".SOYCMS_DB_TYPE.".sql");
