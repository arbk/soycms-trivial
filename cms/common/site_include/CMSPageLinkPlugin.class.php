...
class CMSPageLinkPlugin extends PluginBase
{
    ...
    public function executeReplace($soyValue)
    {
    ...
    public function buildArguments($soyValue)
    {
        $arguments = array(
        "page" => $soyValue,
    ...
    public static function convert($siteRoot, $arguments)
    {
        try {
            ...
            try {
                ...
                if (isset($arguments["site"]) && null!==$arguments["site"]) {
                  // リンク先が他のサイトの場合
...
//              if(!defined("_SITE_ROOT_")) define("_SITE_ROOT_", UserInfoUtil::getSiteDirectory());
            ...
            if ($page->isBlog()) {
                ...
                if ($arguments["month"]) {
                    ...
                    try {
                            $entry = SOY2DAOFactory::create("cms.EntryDAO")->getByIdOnlyAlias($entryId);
                    } catch (Exception $e) {
                          $entry = new Entry();
                    }
