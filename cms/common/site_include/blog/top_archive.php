...
x.x.1～：cms:itemsを追加. ','（カンマ）区切りで出力項目（cms:id）を指定できます.
         未指定（デフォルト）の場合は全ての項目が出力されます.
         ※ b_block:id が複数("id*")ある場合 全て同じ設定となります.
x.x.2～：entry_list_firstを追加. entry_listの1件目のみ格納したリストです.

<!-- b_block:id="entry_list" cms:items="title,content,more_link" -->
<div>
...
function soy_cms_blog_output_entry_list($page, $entries)
{
    ...
    if (!class_exists("BlogPage_EntryList")) {
       ...
       */
        ...
        {
            public $entryCommentDAO;
            public $entryTrackbackDAO;
            ...
            public $_commentDAO;
            public $_trackbackDAO;

            public $outputItems;

            public function getEntryCommentDAO()
            {
                if (!$this->_commentDAO) {
            ...
            public function getEntryTrackbackDAO()
            {
            ...
            public function setEntryPageUrl($entryPageUrl)
            {
            ...
            public function createAddIfOutputItem($id, $className, $array = array())
            {
            ...
            public function addMessagePropertyIfOutputItem($key, $message)
            {
                if (!CMSUtil::isEntryListItems($this->outputItems, $key)) {
                    return;
                }
                $this->addMessageProperty($key, $message);
            }
            ...
            protected function populateItem($entry)
            {
                ...
                $this->createAddIfOutputItem("has_content", "HTMLModel", array(
                "visible" => (0 < strlen($entry->getContent())),
                "soy2prefix"=>"cms"
                ));
                ...
                $this->createAddIfOutputItem("has_more", "HTMLModel", array(
                "visible" => (0 < strlen($entry->getMore())),
                "soy2prefix"=>"cms"
                ));
                ...
                $this->createAddIfOutputItem("trackback_link", "HTMLLink", array(
                "soy2prefix"=>"cms",
                "link" => $link ."#trackback_list",
                "visible" => SOYCMS_ALLOW_BLOG_TRACKBACK
                ));
                $this->createAddIfOutputItem("trackback_count", "CMSLabel", array(
                "soy2prefix"=>"cms",
                "text" => $entry->getTrackbackCount(),
                "visible" => SOYCMS_ALLOW_BLOG_TRACKBACK
                ));
                $this->createAddIfOutputItem("has_trackback", "HTMLModel", array(
                "visible" => (SOYCMS_ALLOW_BLOG_TRACKBACK && 0 < $entry->getTrackbackCount()),
                "soy2prefix"=>"cms"
                ));

                $this->createAddIfOutputItem("comment_link", "HTMLLink", array(
                "soy2prefix"=>"cms",
                "link" => $link ."#comment_list",
                "visible" => SOYCMS_ALLOW_BLOG_COMMENT
                ));
                $this->createAddIfOutputItem("comment_count", "CMSLabel", array(
                "soy2prefix"=>"cms",
                "text" => $entry->getCommentCount(),
                "visible" => SOYCMS_ALLOW_BLOG_COMMENT
                ));
                $this->createAddIfOutputItem("has_comment", "HTMLModel", array(
                "visible" => (SOYCMS_ALLOW_BLOG_COMMENT && 0 < $entry->getCommentCount()),
                "soy2prefix"=>"cms"
                ));

                $this->createAddIfOutputItem("category_list", "BlogPage_EntryList_CategoryList", array(
                ...
                $this->createAddIfOutputItem("visible_by_label", "DisplayCtrlModel", array(
                "entry"=>$entry,
                "ctrlType"=>"label",
                "soy2prefix"=>"cms"
                ));
                for ($lbln = 1; $lbln < SOYCMS_INI_NUMOF_BLOG_DCM_LBL; ++$lbln) {
                    $this->createAddIfOutputItem("visible_by_lab" . $lbln, "DisplayCtrlModel", array(
                    "entry"=>$entry,
                    "ctrlType"=>"label",
                    "soy2prefix"=>"cms"
                    ));
                ...
                // Messageの追加
                $this->addMessagePropertyIfOutputItem("entry_id", '<?php echo $' . $this->_soy2_id . '["entry_id"]; ?>');
                $this->addMessagePropertyIfOutputItem("title", '<?php echo $' . $this->_soy2_id . '["title_plain"]; ?>');
                $this->addMessagePropertyIfOutputItem("content", '<?php echo $' . $this->_soy2_id . '["content"]; ?>');
                $this->addMessagePropertyIfOutputItem("more", '<?php echo $' . $this->_soy2_id . '["more"]; ?>');
                $this->addMessagePropertyIfOutputItem("create_date", '<?php echo $' . $this->_soy2_id . '["create_date"]; ?>');
                $this->addMessagePropertyIfOutputItem("entry_link", '<?php echo $' . $this->_soy2_id . '["entry_link_attribute"]["href"]; ?>');
                $this->addMessagePropertyIfOutputItem("more_link", '<?php echo $' . $this->_soy2_id . '["more_link_attribute"]["href"]; ?>');
                if (SOYCMS_ALLOW_BLOG_TRACKBACK) {
                    $this->addMessagePropertyIfOutputItem("trackback_link", '<?php echo $' . $this->_soy2_id . '["trackback_link_attribute"]["href"]; ?>');
                }
                if (SOYCMS_ALLOW_BLOG_COMMENT) {
                    $this->addMessagePropertyIfOutputItem("comment_link", '<?php echo $' . $this->_soy2_id . '["comment_link_attribute"]["href"]; ?>');
                }
            }
...
//              error_log(var_export($e, true));
    ...
    array_splice($entries, 1);
    $page->createAdd("entry_list_first", "BlogPage_EntryList", array(
    "list" => $entries,
    "entryPageUrl" => $page->getEntryPageURL(true),
    "categoryPageUrl" => $page->getCategoryPageURL(true),
    "blogLabelId" => (int)$page->page->getBlogLabelId(),
        ...
        class BlogPage_EntryListPager extends HTMLList
        {
          ...
          //ベースURL=最初のページのURL
            ...
            public function execute()
            {
                ...
                if (strlen($pager_display_number) ==0) {
                    $pager_display_number = 10;
                }
            ...
            {
                isset($pager_list["url"]) || ($pager_list["url"]=null);
                isset($pager_list["display_number"]) || ($pager_list["display_number"]=null);

                $html = "<a href=\"".soy2_h($pager_list["url"])."\"";

                $class = array();
                if ($pager_list["display_number"] == $this->current) {
                    $class[] = "current_page_number";
                }
                if ($pager_list["display_number"] == 1) {
                    $class[] = "first_page_number";// 1.3.4-
                }
                if ($pager_list["display_number"] == $this->last) {
                    $class[] = "last_page_number";// 1.3.4-
                }
                if (count($class)) {
                ...
                $html .= ">";
                $html .= soy2_h($pager_list["display_number"]);
                $html .= "</a>";

                $this->createAdd("pager_item_link", "HTMLLink", array(
                "link" => soy2_h($pager_list["url"]),
                "text" => soy2_h($pager_list["display_number"]),
                "soy2prefix" => "cms"
                ));
...
function soy_cms_blog_output_prev_next_month($page)
{
  ...
  //年月ではない
    if ((null===$page->year) || (null===$page->month) || null!==$page->day) {
        $isMonthArchive = false;
    }
