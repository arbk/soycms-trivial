...
class DefaultLoginAction extends SOY2Action
{
     ...
     * 転送先が指定されている場合はそこへ
     */
    public function redirectToCMS($siteRole)
    {
        try {
     ...
     * 転送先が指定されている場合はそこへ
     */
    public function redirectToApp($appId)
    {
        //自動ログインなので第1引数はtrue
        UserInfoUtil::loginApp(true);

        if (strlen($this->redirect) >0 && CMSAdminPageController::isAllowedPath($this->redirect, "../app/" . F_FRCTRLER . "/" . $appId)) {
            SOY2PageController::redirect($this->redirect);
        } else {
            SOY2PageController::redirect("../app/" . F_FRCTRLER . "/" . $appId . "?login");
        }
    }
     ...
     */
    public function checkAppRole($appRole)
    {
    ...
    private function getAppRoles($userId)
    {
        ...
        foreach ($appRoles as $appId => $role) {
            if (($appId == "inquiry" && SOYINQUIRY_USE_SITE_DB) ||
                ($appId == "mail" && SOYMAIL_USE_SITE_DB)
            ) {
