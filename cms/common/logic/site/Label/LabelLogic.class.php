...
class LabelLogic extends SOY2LogicBase
{
    ...
    {
        if (!self::checkDuplicateCaption($bean->getCaption())) {
            throw new Exception("Duplicated Caption: ".$bean->getCaption());
        }
     ...
     * キャプションの重複チェック
     */
    public function checkDuplicateCaption($caption, $id = null)
    {
        $dao = self::getLabelDAO();
    ...
    private function getUniqueAlias($caption, $id = null)
    {
        ...
        //重複したら空にしてIDを使う
        if (!self::checkDuplicateAlias($alias, $id)) {
            $alias = null;
        }
     ...
     * @return Array
     */
    public function getWithAccessControl()
    {
        $dao = self::getLabelDAO();
        $labels = $dao->get();
     ...
     * @return Array
     */
    public function getProhibitedLabelIds()
    {
        $prohibitedIds = array();
    ...
    public function getByCaption($caption)
    {
        $dao = self::getLabelDAO();
    ...
    {
        if (!self::checkDuplicateCaption($bean->getCaption(), $bean->getId())) {
            throw new Exception("Duplicated Caption: ".$bean->getCaption());
        }
        if (!self::checkDuplicateAlias($bean->getAlias(), $bean->getId())) {
            throw new Exception("Duplicated Alias: ".$bean->getAlias());
        }

        if ((null===$bean->getAlias()) || !strlen($bean->getAlias())) {
    ...
    public function getBlogCategoryLabelsByPageId($pageId)
    {
    ...
    public function getBlogLabelByPageId($pageId)
    {
        $dao = SOY2DAOFactory::create("cms.BlogPageDAO");
        $labelDAO = SOY2DAOFactory::create("cms.LabelDAO");
        $page = $dao->getById($pageId);
    ...
    private static function &getLabelDAO()
    {
        ...
        if (!$_dao) {
            $_dao = SOY2DAOFactory::create("cms.LabelDAO");
        }
    ...
    public function getNarrowLabels($labelIds)
    {
        ...
        foreach ($tmp as $entryLabel) {
            if (in_array($entryLabel->getLabelId(), $labelIds)) {
                continue;
            }
     ...
     */
    public function getLabelsByBlogPageIdAndEntryId($blogPageId, $entryId)
    {
        static $blogPageLabels, $entryLogic;
        if ((null===$blogPageLabels)) {
        ...
        foreach ($labelIds as $labelId) {
            if (isset($blogPageLabels[$blogPageId][$labelId])) {
                $labelList[] = $blogPageLabels[$blogPageId][$labelId];
            }
        }
