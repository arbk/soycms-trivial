...
class ExImportLogic extends ExImportLogicBase
{
    ...
    public function buildImFunc($items)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            } elseif (preg_match('/custom_search_field\(([^\)]+)\)/', $item, $tmp)) {
                $function[] = '$customSearchFields["' . $tmp[1] . '"] = $item;';
            } elseif (preg_match('/plugins\(([^\)]+)\)/', $item, $tmp)) {
    ...
    public function buildExFunc($items)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            //カスタムサーチフィールド
            } elseif (preg_match('/custom_search_field\(([^\)]+)\)/', $key, $tmp)) {
                $fieldId = $tmp[1];
                ...
                continue;
            //プラグイン
            } elseif (preg_match('/plugins\((.*)\)$/', $key, $tmp)) {
                $pluginId = $tmp[1];
        ...
        $this->_func = function ($obj, $attributes, $customSearchFields, $plugins) use ($function) {
            return eval(implode("\n", $function));
        };
    ...
    {
        if (!$this->entryAttributeDAO) {
            $this->entryAttributeDAO = SOY2DAOFactory::create("cms.EntryAttributeDAO");
        }
        return $this->entryAttributeDAO->getByEntryId($id);
    }

    public function getCustomSearchFieldObject($id)
    {
        $res = array();
        if (file_exists(UserInfoUtil::getSiteDirectory() . ".plugin/CustomSearchField.active")) {
            if (!$this->customSearchFieldDBLogic) {
                $this->customSearchFieldDBLogic = SOY2Logic::createInstance("site_include.plugin.CustomSearchField.logic.DataBaseLogic");
            }
    ...
    public function setCustomSearchFields($customSearchFields)
    {
