...
//          error_log(var_export($e,true));
            error_log("Entry's histories could not be acquired. : " . __METHOD__);
            return array();
        }
    }
...
//              error_log(var_export($e, true));
     ...
     * @throw Exception
     */
    public function getAdditionalArray($entryId)
    {
...
//          error_log(var_export($e,true));
//          error_log("Entries history could not be acquired. : ".__METHOD__);
            return null;
        }
    }
    ...
    private function setChange(EntryHistory $bean, $last)
    {
        if ($last instanceof EntryHistory) {
            if ($bean->getTitle() != $last->getTitle()) {
                $bean->setChangeTitle(1);
            }
            if ($bean->getContent() != $last->getContent()) {
                $bean->setChangeContent(1);
            }
            if ($bean->getMore() != $last->getMore()) {
                $bean->setChangeMore(1);
            }
            if ($bean->getAdditional() != $last->getAdditional()) {
                $bean->setChangeAdditional(1);
            }
            if ($bean->getIsPublished() != $last->getIsPublished()) {
                $bean->setChangeIsPublished(1);
            }
        }
    }
}
