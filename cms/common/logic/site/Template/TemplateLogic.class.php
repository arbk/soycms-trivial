...
class TemplateLogic extends SOY2LogicBase
{
    ...
    public function getByPageType($pageType)
    {
        if (!$this->isSimpleXmlEnabled()) {
            return null;
        }
     ...
     */
    public function installTemplate($id, $installFileList)
    {
...
//                  CMSFileManager::add($path);
     ...
     *
     * @param id
     * @param uninstallFileList アンインストールするファイル名
     */
    public function uninstallTemplate($id, $uninstallFileList)
    {
        $template = $this->getById($id);
        if (!$template->isActive()) {
        ...
        if (!is_array($uninstallFileList)) {
            $uninstallFileList = array();
        }
...
//      SOY2::import("util.CMSFileManager");
     ...
     * テンプレートのアップロードを行う
     */
    public function uploadTemplate($file, $filepath = null)
    {
        if ($filepath) {
        ...
        @chmod(UserInfoUtil::getSiteDirectory() . ".template/" . $newFileName, F_MODE_FILE);
     ...
     */
    public function checkValidTemplatePack($filepath)
    {
        $className = CMSUtil::checkZipEnable(true);
     ...
     * Zip圧縮を行う
     */
    public function createTemplatePack($id, $dir)
    {
     ...
     */
    public function replaceURL($siteUrl, $templateFileList, $fileReplaceList)
    {
     ...
     */
    public function replaceSiteRootMarker($path)
    {
