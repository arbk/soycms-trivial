...
class ImageLogic extends SOY2LogicBase
{
    ...
    public function resize($path, $width, $height, $new_path = null, $flag = 0)
    {
        $info = pathinfo($path);
     ...
     * @return new_filepath
     */
    public function makeThumbnail($path, $width, $height, $flag = false)
    {
        $name = basename($path);
     ...
     * @return new_filepath
     */
    public function trimImage($path, $width, $height, $type = "middle")
    {
        $name = basename($path);
        $new_path = preg_replace('/'.$name.'$/', "thumbnail_".$width."x".$height.'_'.$name, $path);
