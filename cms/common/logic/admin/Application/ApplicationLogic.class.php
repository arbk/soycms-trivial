...
class ApplicationLogic extends SOY2LogicBase
{
    ...
    private function importApplicationIniFile($appId)
    {
        ...
        $inis = parse_ini_file($iniFile, false, INI_SCANNER_RAW);
        if (!$inis) {
            return false;
        }
     ...
     */
    public function getLoginableApplications($userId)
    {
        static $app_res;
        if ((null===$app_res)) {
            $app_res = array();
            ...
            if (is_array($applications) && count($applications)) {
                ...
                foreach ($roles as $userId => $role) {
                    ...
                    if ($appId == "inquiry" && defined("SOYINQUIRY_USE_SITE_DB") && SOYINQUIRY_USE_SITE_DB) {
                        continue;
                    }
                    if ($appId == "mail" && defined("SOYMAIL_USE_SITE_DB") && SOYMAIL_USE_SITE_DB) {
                        continue;
                    }
    ...
    public function getLoginiableApplicationLists()
    {
        if (UserInfoUtil::isDefaultUser()) {
            return $this->getApplications();
        } else {
            return $this->getLoginableApplications(UserInfoUtil::getUserId());
        }
    }
}
