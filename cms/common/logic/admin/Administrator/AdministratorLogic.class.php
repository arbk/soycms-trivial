...
//          $this->upgradeAdministratorPassword($bean, $password);
    ...
    public function setAutoLogin($userId)
    {
        ...
        $expire = SOYCMS_AUTOLOGIN_EXPIRE * 24 * 60 * 60 + SOYCMS_NOW;

        $domain = $_SERVER["HTTP_HOST"];
        if (strpos($domain, ":")) {
            $domain = substr($domain, 0, strpos($domain, ":"));  //portがある場合は削除
        }

        soy2_setcookie("sc_auto_login", $token, array("expires"=>$expire,"domain"=>$domain));
     ...
     */
    public function getLimitedAdministratorList()
    {
     ...
     */
    public function updateAdministratorPassword($userid, $newPassword)
    {
        $dao = SOY2DAOFactory::create("admin.AdministratorDAO");
     ...
     * @param password
     */
    public function checkUserIdAndPassword($id, $password)
    {
        $dao = SOY2DAOFactory::create("admin.AdministratorDAO");
        try {
     ...
     */
    public function createAdministrator($id, $password, $isDefault = false, $name = "", $email = "")
    {
        $dao = SOY2DAOFactory::create("admin.AdministratorDAO");
...
//          return true;
        ...
        foreach ($configs as $config) {
            if (!isset($_POST["custom_field"][$config->getFieldId()])) {
                continue;
            }
     ...
     * @param id 管理者のID
     */
    public function checkDefaultUser($id)
    {
        $dao = SOY2DAOFactory::create("admin.AdministratorDAO");
        try {
...
//          }
        ...
        if (strlen($str)==0) {
            throw new Exception("Failed to generate random strings.");
        }
