...
class UserInfoUtil implements IUserInfoUtil
{
    ...
    {
        if (SOYCMS_LOGIN_LIFETIME > 0) {
            // 一定時間アクセスがなかったらログアウトする
            $lastAccessTime = SOY2ActionSession::getUserSession()->getAttribute("lastAccessTime");
            if ($lastAccessTime + SOYCMS_LOGIN_LIFETIME < SOYCMS_NOW) {
                self::logout();
            }
        }

        SOY2ActionSession::getUserSession()->setAttribute("lastAccessTime", SOYCMS_NOW);
    ...
    {
        return soy2_h(SOY2ActionSession::getUserSession()->getAttribute("username"));
    }
    ...
    {
        return soy2_h(SOY2ActionSession::getUserSession()->getAttribute("email"));
    }
    ...
    {
        if ((null===$siteId)) {
            $siteId = self::getSite()->getSiteId();
        }

        $url = empty(SOYCMS_TARGET_URL) ? SOY2PageController::createRelativeLink("/", true) : SOYCMS_TARGET_URL;
