...
class SOY2PageController implements SOY2_Controller
{
   ...
   */
    ...
    {
        $html = '<!DOCTYPE html><title>404 Not Found</title><p>404 Not Found.';
...
class SOY2_PathInfoPathBuilder implements SOY2_PathBuilder
{
   ...
   */
    public function createLinkFromPath($path, $isAbsoluteUrl = false)
    {
   ...
   */
    public function createLinkFromRelativePath($path, $isAbsoluteUrl = false)
    {
    ...
    protected static function getScriptPath()
    {
        ...
        if (!$script) {
           ...
           */
            ...
            if (strlen($documentRoot) > 0 && $documentRoot[strlen($documentRoot) - 1] != "/") {
                $documentRoot .= "/";
            }
   ...
   */
    protected static function createAbsoluteURL($path)
    {
        ...
        if (null===$domain && isset($_SERVER["SERVER_NAME"])) {
            $domain = $_SERVER["SERVER_NAME"];
        }
        if (null===$port) {
            if (isset($_SERVER["SERVER_PORT"]) && strlen($_SERVER["SERVER_PORT"]) > 0
            && ( ("http"===$scheme && $_SERVER["SERVER_PORT"] != "80")
            || ("https"===$scheme && $_SERVER["SERVER_PORT"] != "443") )
            ) {
   ...
   */
    protected static function convertRelativePathToAbsolutePath($relativePath, $base)
    {
        ...
        if (count($dirs) > 0 && $dirs[count($dirs)-1] == "") {
            array_pop($dirs);
        }
...
class SOY2Mail
{
    ...
    public function getEncodedSubject()
    {
        if (strlen($this->encodedSubject) < 1) {
            $this->encodedSubject = mb_encode_mimeheader(
                $this->subject,
                $this->getSubjectEncodingForConvert(),
                "B",
                "\r\n",
                strlen("Subject: ")
            );
    ...
    public function setEncodedSubject($encodedSubject)
    {
    ...
    public function setText($text, $encoding = null)
    {
        ...
        if (!$this->encodedText) {
            if (!$encoding) {
                $encoding = $this->getEncodingForConvert();
            }
    ...
    public function setFrom($from, $label = null, $encoding = null)
    {
        if (!$encoding) {
    ...
    public function getEncodedRecipients()
    {
        return $this->encodedRecipients;
    }
    public function setEncodedRecipients($encodedRecipients)
    {
   ...
   * 受信者を追加する
   */
    public function addRecipient($address, $label = null, $encoding = null)
    {
        if (!$encoding) {
            $encoding = $this->getEncoding();
        }
   ...
   * 受信者を削除する
   */
    public function removeRecipient($address)
    {
   ...
   * BCC受信者を追加する
   */
    public function addBccRecipient($address, $label = null, $encoding = null)
    {
        if (!$encoding) {
            $encoding = $this->getEncoding();
        }
   ...
   * BCC受信者を削除する
   */
    public function removeBccRecipient($address)
    {
   ...
   * 添付ファイルを追加する
   */
    public function addAttachment($filename, $type, $contents)
    {
   ...
   */
    ...
    public function setSubjectEncoding($subjectEncoding)
    {
   ...
   * 文字コード変換のための本文の文字コード
   */
    public function getEncodingForConvert()
    {
   ...
   * 文字コード変換のための件名の文字コード
   */
    public function getSubjectEncodingForConvert()
    {
...
class SOY2Mail_MailAddress
{
    ...
    private $label;
    private $encoding;
    public function __construct($address, $label = "", $encoding = "")
    {
...
class SOY2Mail_IMAPLogic extends SOY2Mail implements SOY2Mail_ReceiverInterface
{
    ...
    public function __construct($options)
    {
        ...
        if (isset($options["imap.flag"])) {
            $this->flag = $options["imap.flag"];
        }
        if (isset($options["imap.folder"])) {
            $this->folder = $options["imap.folder"];
        }
    ...
    public function receive()
    {
        ...
        if ($encoding) {
            $mail->setText(mb_convert_encoding($body, SOY2::CHARSET, $encoding));
        } else {
            $mail->setText(mb_convert_encoding($body, SOY2::CHARSET, "JIS,SJIS,EUC-JP,UTF-8,ASCII"));
        }
   ...
   */
    public function getParameterValue($parameters, $attribute)
    {
...
class SOY2Mail_POPLogic extends SOY2Mail implements SOY2Mail_ReceiverInterface
{
    ...
    public function __construct($options)
    {
        ...
        if (isset($options["pop.flag"])) {
            $this->flag = $options["pop.flag"];
        }
        if (isset($options["pop.folder"])) {
            $this->folder = $options["pop.folder"];
        }
    ...
    public function open()
    {
        ...
        if (!$buff) {
            throw new SOY2MailException("Failed to connect pop server");
        }
    ...
    public function receive()
    {
        ...
        if (!$res) {
            throw new SOY2MailException("failed to open Receive Server");
        }
        $mailId = null;
        while (true) {
    ...
    public function getPopResponse()
    {
        $buff = fgets($this->con);
        $buff = rtrim($buff, "\r\n");
        return $buff;
...
class SOY2Mail_SMTPLogic extends SOY2Mail implements SOY2Mail_SenderInterface
{
        ...
        if ($this->isSMTPAuth && isset($this->esmtpOptions["AUTH"]) && is_array($this->esmtpOptions["AUTH"])) {
          ...
          /** CRAM-MD5を最優先にしてDIGEST-MD5の優先度を下げる **/
            if (in_array("CRAM-MD5", $authTypes)) {
                ...
                $buff = $this->getSmtpResponse();
                if (strlen($buff) < 5 || substr($buff, 0, 3) != "334") {
                ...
                while (true) {
                    $buff = $this->getSmtpResponse();
                    if (substr($buff, 0, 3) == "235") {
                        break;
                    }
                    if (substr($buff, 0, 3) == "501") {
                        throw new SOY2MailException("smtp login failed: wrong parameter");
                    }
                    if (substr($buff, 0, 3) == "535") {
                        throw new SOY2MailException("smtp login failed: wrong id or password");
                    }
                }
            } elseif (in_array("DIGEST-MD5", $authTypes)) {
    ...
    public function sendMail(SOY2Mail_MailAddress $sendTo, $bccRecipients = array())
    {
        $sent = false;
        $try = $try_connect = 0;
        while (!$sent) {
            ...
            while (!$this->con) {
                ...
                try {
                    ...
                    if ($try_connect > 10) {
                        if ($this->debug) {
                            echo "SMTP Failed to open SMTP connection.\n";
                        }
                        throw $e;
                    }
                }
    ...
    private function _sendMail(SOY2Mail_MailAddress $sendTo, $bccRecipients = array())
    {
        ...
        while (true) {
            ...
            if (strlen($str) && substr($str, 0, 3) != "250") {
                throw new SOY2MailException("Failed: MAIL FROM " . $str);
            }
        }
        ...
        while (true) {
            ...
            if (substr($str, 0, 3) != "250") {
                throw new SOY2MailException("Failed: RCPT TO " . $str);
            }
        }
        ...
        while (true) {
            ...
            if (substr($str, 0, 3) != "250") {
                throw new SOY2MailException("Failed: DATA " . $str);
            }
        }
    ...
    public function smtpCommand($string)
    {
        if (!$this->con) {
    ...
    public function getIsSMTPAuth()
    {
        return $this->isSMTPAuth;
    }
    public function setIsSMTPAuth($isSMTPAuth)
    {
...
{
    public static function getResponse($user, $pass, $challengeStr)
    {
...
{
    public static function getResponse($user, $pass, $challengeStr, $hostname)
    {
        $challenge = array();
...
class SOY2Mail_SendMailLogic extends SOY2Mail implements SOY2Mail_SenderInterface
{
    ...
    public function sendMail($sendTo, $bccRecipients = array())
    {
        $to = $sendTo->getString();
        $from = $this->getFrom();
        $title = $this->getEncodedSubject();
...
class SOY2Mail_ServerConfig
{
   ...
   */
    public function buildReceiveMail()
    {
   ...
   */
    ...
    public function setReceiveServerType($receiveServerType)
    {
    ...
    public function getFromMailAddressName()
    {
        return $this->fromMailAddressName;
    }
    public function setFromMailAddressName($fromMailAddressName)
    {
    ...
    public function getReturnMailAddressName()
    {
        return $this->returnMailAddressName;
    }
    public function setReturnMailAddressName($returnMailAddressName)
    {
...
class SOY2Action extends SOY2ActionBase
{
   ...
   * Action実行前準備
   */
    protected function prepareImpl(
        SOY2ActionRequest  &$request,
   ...
   */
    ...
    {
        if (method_exists($this, "clearance")) {
            $this->clearance($response);
        }
        $response = null;
    }
  /**
...
{
    public static function &createInstance($path, $attributes = array())
    {
        $obj = null;
        if (class_exists($path)) {
            $obj = new $path();
            ...
            if (!class_exists($className)) {
                ...
                if (defined("SOY2ACTION_AUTO_GENERATE") && SOY2ACTION_AUTO_GENERATE && !file_exists($fullPath)) {
    ...
    private static function generateAction($className, $fullPath, $attributes)
    {
        ...
        while (realpath($dirpath) == false) {
            ...
            if (defined("F_MODE_DIR")) {
                @chmod($dirpath, F_MODE_DIR);
            }
...
class SOY2ActionForm
{
   ...
   * @return boolean
   */
    public function isError($propName)
    {
   ...
   */
    public function getErrorString($propName)
    {
   ...
   * エラーを設定
   */
    public function setError($propName, ActionFormError $error)
    {
...
class ActionFormError
{
   ...
   */
    public function __construct($class, $prop, $validator, $error, $message = null)
    {
...
abstract class SOY2ActionFormValidator
{
   ...
   */
    public static function getValidator($param, ReflectionProperty &$property, ReflectionMethod &$reflectionMethod)
    {
        ...
        $tmp = array();
        if (!preg_match('/@validator\s+([^\s]*)(?:\s+(\{.*\}))?/m', $comment, $tmp)) {
            return null;
        }
        $type = $tmp[1];
        $json = @$tmp[2];
    ...
    abstract public function validate(SOY2ActionForm &$form, $propName, $value, $isRequire);
}
...
class SOY2UserSession extends SOY2ActionSessionBase
{
    ...
    public function setAuthenticated($key, $flag = null)
    {
        if (null===$flag && is_bool($key)) {
            $flag = $key;
            $key = "default";
        }
      /**
    ...
    public function getAuthenticated($key = null)
    {
        if (null===$key) {
            $key = "default";
        }
...
class SOY2DAOConfig
{
    ...
    public $pass = '';
    public $daoDir = "dao/";
    public $entityDir = "entity/";
    public $daoCacheDir; // Daoのキャッシュはディフォルトは行わない
    public $event = array();
  /*
   ...
   */
    ...
    public static function setTableMapping($key, $value = null)
    {
        $config = self::getInstance();
        if ($value) {
            $config->tableMappings[$key] = $value;
        }
   ...
   */
    ...
    {
        $config = self::getInstance();
        if (!isset($config->event["query"])) {
            $config->event["query"] = array();
        }
    ...
    {
        $config = self::getInstance();
        if (!isset($config->event["updateQuery"])) {
            $config->event["updateQuery"] = array();
        }
...
class SOY2DAO
{
   ...
   * @return bind配列
   */
    public function buildBinds($sql, $binds)
    {
   ...
   * @return SQL文
   */
    public function &buildQuery($method, $noPersistents = array(), $columns = array(), $queryType = null)
    {
        if (!isset($this->_query[$method])) {
            $this->_query[$method] =
            SOY2DAO_QueryBuilder::buildQuery($method, $this->getEntityInfo(), $noPersistents, $columns, $queryType);
            $this->_query[$method]->ignoreColumns($this->_ignoreColumns);
   ...
   */
    ...
    public function releaseDataSource()
    {
   ...
   * @return 結果配列
   */
    public function executeQuery($query, $binds = array(), $keepStatement = false)
    {
        if ($query instanceof SOY2DAO_Query) {
            if (strlen($this->getOrder())) {
                $query->setOrder($this->getOrder());
            }
            $query->ignoreColumns($this->_ignoreColumns);
            $query->replaceTableNames();
            $sql = $query->getQuery();
        } else {
            $sql = $query;
        }
        if (null!==$this->_keepStatement) {
            $keepStatement = $this->_keepStatement;
        }
        $isUseLimitQuery = false;
        if (SOY2DAOConfig::getOption("limit_query") === true && null!==$this->_limit) {
        ...
        if ($isUseLimitQuery) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                if ((null===$this->_limit) || $counter < ($this->_offset + $this->_limit)) {
                    $row = $this->unquoteColumnName($query, $sql, $row);
                    $resultArray[] = $row;
                }
                $counter++;
   ...
   * @return 結果
   */
    public function executeUpdateQuery($sql, $binds = array(), $keepStatement = false)
    {
        if ($sql instanceof SOY2DAO_Query) {
            if (strlen($this->getOrder())) {
                $sql->setOrder($this->getOrder());
            }
            $sql->ignoreColumns($this->_ignoreColumns);
   ...
   */
    public function setKeepStatement($flag)
    {
...
class SOY2DAOException extends Exception
{
    ...
    public function getPDOExceptionMessage()
    {
    ...
    public function setPdoException($pdoException)
    {
...
class SOY2DAOFactoryImpl extends SOY2DAOFactory
{
   ...
   */
    public static function build($className)
    {
        ...
        $str .= "\n";
        $str .= 'public $_entity = "' . str_replace('"', '\"', serialize($entityInfo)) . '";';
        $str .= "\n";
        $str .= implode("\n", $methodStrings);
        $str .= "}";
      /*
       ...
       */
        if (SOY2DAOConfig::DaoCacheDir()) {
            ...
            fwrite($fp, "<?php\n } \n?>");
            fclose($fp);
            if (defined("F_MODE_FILE")) {
                @chmod($cacheFilePath, F_MODE_FILE);
            }
        }
        eval($str);
    ...
    public static function buildMethod($method, $entityInfo)
    {
        ...
        foreach ($parameters as $param) {
            ...
            if (method_exists($param, "getType")) {
                $class = (null!==$param->getType() && !$param->getType()->isBuiltin()) ? new ReflectionClass($param->getType()->getName()) : null;
            } else {
        ...
        '"' . $queryType . '");';
        if ($table) {
            $methodString[] = '$query->table = "' . $table . '";';
        }
        if ($queryAnnotation) {
            $methodString[] = '$query->where = "' . $queryAnnotation . '";';
        }
        if ($order) {
            $methodString[] = '$query->order = "' . $order . '";';
        }
        if ($group) {
            $methodString[] = '$query->group = "' . $group . '";';
        }
        if ($having) {
            $methodString[] = '$query->having = "' . $having . '";';
        }
        if ($distinct) {
            $methodString[] = '$query->distinct = true;';
        }
        $props = array();
        ...
        if ($method->isAbstract()) {
            ...
            } elseif (preg_match("/^delete|^remove/", strtolower($method->getName())) || $queryType == "delete") {
                $methodString[] = '$result = $this->executeUpdateQuery($query,$binds);';
            } elseif (preg_match("/^update|^save|^write|^reset|^change/", strtolower($method->getName())) || $queryType == "update") {
                $methodString[] = '$result = $this->executeUpdateQuery($query,$binds);';
            } else {
...
class SOY2DAO_Entity
{
    ...
    public $reverseColumns = array();  //逆引きテーブル
  /**
   * @return EntityClassのProperty名を連想配列のキーとし、値にカラム名が入ったArrayを返す
   * @param readOnlyな属性も取得するかどうか
   */
    public function getColumns($flag = false)
    {
        $array = array();
        foreach ($this->columns as $column) {
            if (!$flag && $column->readOnly) {
                continue;
            }
   ...
   * @param $name カラム名
   */
    public function getColumnByName($name, $isThrow = true)
    {
   ...
   * 逆引きテーブルを作成
   */
    public function buildReverseColumns()
    {
...
{
    public $id; // identity,またはsequence=シーケンス名
    public $name; // カラム名?
    public $alias; // 変換後のカラム名
    public $prop;
    public $isPrimary;
    public $readOnly;
    public $sequence;
...
class SOY2DAO_Query
{
   ...
   */
    public function __toString()
    {
        ...
        switch ($this->prefix) {
            case "insert":
                $sql = "insert into " . $this->quoteIdentifier($this->table) . " " . $this->sql;
            ...
            case "select":
                ...
                break;
            case "update":
                $sql = "update " . $this->quoteIdentifier($this->table) . " set " . $this->sql;
                if (strlen($this->where)) {
                    $sql .= " where " . $this->where;
                }
                break;
   ...
   */
    public function parseExpression($arguments)
    {
      /*
       ...
       */
        ...
        if (preg_match($phpExpression, $this->where, $tmp)) {
            ...
            if (!is_string($replace) and !is_numeric($replace)) {
                throw new SOY2DAOException("PHP式の変換に失敗しました。(" . $tmp[1] . ")");
            }
   ...
   * （使われていない模様）
   */
    public function replaceTableNames()
    {
    ...
    public function replaceTableName($key)
    {
    ...
    public static function getQuotes()
    {
        switch (SOY2DAOConfig::type()) {
            case SOY2DAOConfig::DB_TYPE_MYSQL:
                return self::IDENTIFIER_QUALIFIER_MYSQL;
            case SOY2DAOConfig::DB_TYPE_SQLITE:
                return self::IDENTIFIER_QUALIFIER_SQLITE;
            case SOY2DAOConfig::DB_TYPE_POSTGRES:
                return self::IDENTIFIER_QUALIFIER_POSTGRES;
            default:
                return "";
        }
    }
   ...
   */
    ...
    public function quoteIdentifiers($identifiers)
    {
        ...
        if (!is_array($identifiers)) {
            $isStr = true;
            $identifiers = explode(",", (string)$identifiers);
        }
        foreach ($identifiers as $k => $v) {
            $identifiers[$k] = $this->quoteIdentifier($v);
        }
        if ($isStr) {
    ...
    public function quoteSql()
    {
        $this->sql = $this->quoteIdentifiers($this->sql);
    }
  /**
   * 識別子の引用符を外す
   */
    ...
    public function ignoreColumns($ignoreColumns)
    {
        if (!is_array($ignoreColumns) || 1 > count($ignoreColumns)) {
            return;
        }
        switch ($this->prefix) {
            case "insert":
          ...
          //        $this->sql = preg_replace("/(^|,)(".$column.")(,|$)/i", "$1NULL AS $2$3", $this->sql); // NULL方式
                    $this->sql = trim(preg_replace("/(^|,)(".$column.")(,|$)/i", "$3", $this->sql), ",");  // 削除方式
                }
                break;
            case "update":
              // implement as necessary
                break;
            case "delete": // unnecessary for delete
                break;
        }
    }
  /**
   ...
   */
    public function __call($name, $args)
    {
      ...
      **/
        ...
        if (!isset($this->_soy2_functions[$name])) {
            return null;
        }
...
//const ENCODING = 'UTF-8';
   ...
   * 前置詞を設定
   */
    public function setSoy2Prefix($prefix)
    {
   ...
   * setter parent
   */
    public function setParentObject($obj)
    {
   ...
   */
    public function setParentPageParam($param)
    {
   ...
   */
    public function getParentPageParam()
    {
   ...
   */
    ...
    {
        return $this::SOY_TYPE; // for PHP >= 5.3.0

      // $func = function(){
      //   //PHP5.3でも一応動作する　調査用
      //   if((null===$this) || is_string($this)) return "_HTML_BODY_";
   ...
   */
    ...
    public function setSkipEndTag($boolean)
    {
        $this->_skip_end_tag = $boolean;
    }
    public function getIsSkipEndTag()
    {
   ...
   */
    public function parse($suffix, $value, $content)
    {
        $result = array(
        "tag"=>"",
        "line"=>"",
        ...
        if ($content instanceof HTMLList_DummyObject) {
            $content = "";
        }
        switch ($this->getComponentType()) {
            case SOY2HTML::HTML_BODY:
                ...
                if (preg_match($regex, $content, $tmp, PREG_OFFSET_CAPTURE)) {
                    ...
                    } elseif (preg_match($endRegex, $content, $tmp2, PREG_OFFSET_CAPTURE)
            ...
            case SOY2HTML::SOY_BODY:
                ...
                } elseif (preg_match($startRegex, $content, $tmp1, PREG_OFFSET_CAPTURE)) {
                    $tag = $tmp1[3][0];
   ...
   * @param $line
   */
    public function parseAttributes($line)
    {
        ...
        if (preg_match_all($regex, $line, $tmp)) {
            ...
            foreach ($keys as $i => $key) {
                $key = strtolower($key);
                $value = html_entity_decode($values[$i], ENT_QUOTES, SOY2::CHARSET);
   ...
   */
    public function getContent(SOY2HTML $tag, $content)
    {
   ...
   */
    ...
    //    if( $this->getComponentType() == SOY2HTML::SKIP_BODY OR $this->getIsSkipEndTag() ){
            if ($this->getComponentType() != SOY2HTML::SKIP_BODY && $this->getIsSkipEndTag()) {
                $out .= ' /';
            }
        }
        $out .= '>';
   ...
   */
    public function getVisbleScript()
    {
        return array(
   ...
   *
   */
    public function setAttribute($key, $value, $flag = true)
    {
        $key = strtolower($key);
   ...
   * 属性値を保存する
   */
    public function setPermanentAttribute($key, $value)
    {
   ...
   * 保存した属性値を取得する
   */
    public function getPermanentAttribute($key = null)
    {
        if (null===$key) {
   ...
   */
    ...
    public function addMessageProperty($key, $message)
    {
   ...
   */
    public function parseMessageProperty()
    {
        if ($this->getIsModified()) {
   ...
   * @param $obj タグ
   */
    public function set($id, SOY2HTML &$obj, &$page = null)
    {
        if (null===$page) {
        ...
        foreach ($attribute as $key => $value) {
            ...
            $value = (string)$value;
            if (strlen($value)) {
                $page[$obj->getId() . "_attribute"][$key] = soy2_h($value);
            } else {
   ...
   */
    public static function ToText($html, $encoding = SOY2::CHARSET)
    {
      /*
...
class SOY2HTMLFactory extends SOY2HTMLBase
{
   ...
   */
    public static function importWebPage($className)
    {
        ...
        $extension = ".class.php";
        require_once($pageDir . $path . $extension);
    }
    public static function pageExists($className)
    {
        ...
        $path = str_replace(".", "/", $className);
        $extension = ".class.php";
        if (defined("SOY2HTML_AUTO_GENERATE") && SOY2HTML_AUTO_GENERATE && !file_exists($pageDir . $path . $extension) && file_exists($pageDir . $path . ".html")) {
...
class SOYBodyComponentBase extends SOY2HTML
{
   ...
   * @see HTMLPage.add
   */
    public function createAdd($id, $className, $array = array())
    {
    ...
    public function setChildSoy2Prefix($prefix)
    {
  ...
  /** PHP7.4対応 SOY2HTMLBaseの__call()の廃止 **/
    public function addForm($id, $array = array())
    {
        $this->createAdd($id, "HTMLForm", $array);
    }
    public function addUploadForm($id, $array = array())
    {
        $this->createAdd($id, "HTMLUploadForm", $array);
    }
    public function addModel($id, $array = array())
    {
        $this->createAdd($id, "HTMLModel", $array);
    }
    public function addLabel($id, $array = array())
    {
        $this->createAdd($id, "HTMLLabel", $array);
    }
    public function addImage($id, $array = array())
    {
        $this->createAdd($id, "HTMLImage", $array);
    }
    public function addLink($id, $array = array())
    {
        $this->createAdd($id, "HTMLLink", $array);
    }
    public function addActionLink($id, $array = array())
    {
        $this->createAdd($id, "HTMLActionLink", $array);
    }
    public function addInput($id, $array = array())
    {
        $this->createAdd($id, "HTMLInput", $array);
        $this->addText($id, $array);
    }
    public function addTextArea($id, $array = array())
    {
        $this->createAdd($id, "HTMLTextArea", $array);
        $this->addText($id, $array);
    }
    public function addCheckBox($id, $array = array())
    {
        $this->createAdd($id, "HTMLCheckBox", $array);
        $this->addText($id, $array);
    }
    public function addSelect($id, $array = array())
    {
        $this->createAdd($id, "HTMLSelect", $array);
        $this->addText($id, $array);
    }
    public function addHidden($id, $array = array())
    {
        $this->createAdd($id, "HTMLHidden", $array);
    }
    public function addScript($id, $array = array())
    {
        $this->createAdd($id, "HTMLScript", $array);
    }
    public function addCSS($id, $array = array())
    {
        $this->createAdd($id, "HTMLCSS", $array);
    }
    public function addCSSLink($id, $array = array())
    {
        $this->createAdd($id, "HTMLCSSLink", $array);
    }
    public function addText($id, $array = array())
    {
        $new = array();
        if (isset($array["soy2prefix"]) && strlen($array["soy2prefix"])) {
            $new["soy2prefix"] = $array["soy2prefix"];
        }
        $new["text"] = (isset($array["value"])) ? $array["value"] : null;
        if (!strlen($new["text"]) && isset($array["text"]) && strlen($array["text"])) {
            $new["text"] = $array["text"]; //addTextAreaの場合
        }
        $this->createAdd($id. "_text", "HTMLLabel", $new);
    }
    public function addList($id, $array = array())
    {
        $this->createAdd($id, "HTMLList", $array);
    }
}
/**
 * @package SOY2.SOY2HTML
...
class SOY2HTMLElement extends SOY2HTML
{
    ...
    public function appendChild(SOY2HTMLElement &$ele)
    {
        $this->_elements[] = $ele;
    }
}
...
class SOY2HTMLStyle
{
    ...
    public function __set($key, $value)
    {
      //$key = preg_replace_callback('/[A-Z]/',create_function('$word','return \'-\'.strtolower($word[0]);'),$key);
        $key = preg_replace_callback('/[A-Z]/', function ($word) use ($key) {
...
class HTMLCSS extends SOY2HTML
{
    ...
    public function setStyle($text)
    {
        $this->text = $text;
    }
    public function getObject()
    {
        return $this->text; // soy2_h((string)$this->text);
    }
}
...
class HTMLForm extends SOYBodyComponentBase
{
    ...
    public function getStartTag()
    {
        if ($this->_generateToken && strtolower($this->_method) == "post") {
            $token = '<input type="hidden" name="s_token" value="<?php echo soy2_get_token(); ?>">';
...
class HTMLSelect extends HTMLFormElement
{
    ...
    const SOY_TYPE = SOY2HTML::HTML_BODY;
    public $options;
    public $selected; // 複数指定するときは配列
    private $multiple = false;
    public $indexOrder = false;
    public $property;
    public $each = "";
    ...
    public function setEach($each)
    {
        if (is_array($each) && count($each)) {
            ...
            foreach ($each as $key => $value) {
                $attr[] = soy2_h((string)$key) . '="' . soy2_h((string)$value) . '"';
            }
    ...
    public function getObject()
    {
        ...
        if ($twoDimensional && is_array($this->options) && count($this->options)) {
            foreach ($this->options as $key => $value) {
                if (is_array($value)) {
                    $key = (string)$key;
                    $buff .= '<optgroup label="' . soy2_h((string)$key) . '">';
    ...
    public function buildOptions($options, $isHash)
    {
        $buff = "";
        if (is_array($options)) {
    ...
    public function buildOption($key, $value, $isHash)
    {
        $buff = "";
        $selected = '';
        $key = (string)$key;
        ...
        if (strlen($this->each)) {
            $attributes .= " " . $this->each;
        }
        if ($isHash || !is_numeric($key)) {
            $attributes .= ' value="' . soy2_h((string)$key) . '"';
        }
        $buff .= "<option" . $attributes . ">" . soy2_h((string)$value) . "</option>";
        return $buff;
    }
  /**
   * 値がselectedであるかどうか
...
class HTMLCheckBox extends HTMLInput
{
    ...
    public function execute()
    {
        parent::execute();
        if (!$this->elementId) {
            $this->elementId = "label_" . md5((string)$this->value.(string)$this->name.(string)rand(0, 1));
        }
    ...
    public function getObject()
    {
        return soy2_h($this->getLabel());
    }
    public function setIsBoolean($flag)
    {
        $this->isBoolean = $flag;
    }
...
class HTMLHead extends SOY2HTML
{
    ...
    {
        return soy2_h((string)$this->title);
    }
    public function setIsEraseHead($boolean)
    {
        $this->isEraseHead = (boolean)$boolean;
    }
    public function getIsEraseHead()
    {
    ...
    public function getMetaHTML()
    {
        ...
        foreach ($metas as $akey => $avalue) {
            ...
            foreach ($avalue as $key => $value) {
                $attributes[$key] = $key . '="' . soy2_h((string)$value) . '"';
            }
    ...
    public function getScriptHTML()
    {
        ...
        //        $body = "<!--\n" . $value . "\n-->"; // scriptの中身はHTML4ではCDATA, XHTML1だとPCDATA
            ...
            if (!array_key_exists("charset", $attributes)) {
                $attributes["charset"] = 'charset="' . SOY2::CHARSET . '"';
            }
...
class HTMLLabel extends SOY2HTML
{
    ...
    public function setIsFolding($flag)
    {
        $this->isFolding = (boolean)$flag;
    }
    public function setFoldingTag($tag)
    {
        $this->foldingTag = $tag;
    }
...
class HTMLActionLink extends HTMLLink
{
    public function execute()
    {
        ...
        $link .= "s_token=" . rawurlencode(soy2_get_token());
...
class HTMLList extends SOYBodyComponentBase
{
    ...
    public function populateItemImpl($entity, $key, $counter, $length)
    {
...
//const CACHE_CONTENTS_LENGTH_MIN = 81;
   ...
   */
    public function prepare()
    {
        ...
        if ($content !== false && strlen($content)) {
           ...
           * ただしXML宣言は残す
           */
            $content = self::checkAndEscapePhpTag($content);
          /*
    ...
    public static function checkAndEscapePhpTag($content)
    {
        if (defined("SOY2HTML_ALLOW_PHP_SCRIPT") && SOY2HTML_ALLOW_PHP_SCRIPT == false) {
            $content = soy2_escape_phptag($content);
        }
        return $content;
    }
    public function getBodyElement()
    {
        if (null===$this->_soy2_body_element) {
            $this->_soy2_body_element = new HTMLPage_ChildElement("body");
        }
        return $this->_soy2_body_element;
    }
    public function getHeadElement()
    {
        if (null===$this->_soy2_head_element) {
            $this->_soy2_head_element = new HTMLPage_HeadElement("head");
        }
   ...
   * @param 初期値
   */
    public function create($id, $className, $array = array())
    {
        if (is_object($className)) {
            $obj = $className;
            $obj->setId($id);
   ...
   */
    ...
    {
        if ($this->_soy2_body_element) {
            $this->_soy2_content = $this->_soy2_body_element->convert($this->_soy2_content, $this->getPageParam());
        }
        if ($this->_soy2_head_element) {
            $this->_soy2_content = $this->_soy2_head_element->convert($this->_soy2_content, $this->getPageParam());
        }
        $page = &$this->_soy2_page;
        if ($this->_soy2_body_element) {
            $page = $this->_soy2_body_element->execute($page);
        }
        if ($this->_soy2_head_element) {
            $page = $this->_soy2_head_element->execute($page);
        }
        $this->parsePlugin();
   ...
   * 永続化の属性値を作成
   */
    public function createPermanentAttributesCache()
    {
  ...
  //  }
        ...
        $reflection = new ReflectionClass(get_class($this));
        $classFilePath = $reflection->getFileName();
        if (defined("SOY2HTML_CACHE_FORCE") && SOY2HTML_CACHE_FORCE) {
            return true;
        }
...
class HTMLPage_ChildElement
{
    ...
    public function convert($html, $pageParam)
    {
        if ($html != false) {
...
class HTMLPage_HeadElement extends HTMLPage_ChildElement
{
   ...
   */
    public function clearMeta($name)
    {
        if (isset($this->metas[$name])) {
    ...
    public function convert($html, $pageParam)
    {
        if ($html != false) {
            ...
            && preg_match('/(<\/title\s?[^>]*>)/i', $html, $tmp2, PREG_OFFSET_CAPTURE)
                ...
                $out = $tmp1[1][0] . '<?php echo soy2_h($' . $pageParam . '["page_' . $this->tag . '_title"]); ?>' . $tmp2[1][0];
            ...
            foreach ($meta[1] as $key => $array) {
                ...
                if (preg_match('/name\s*=\s*"([^"]+)"/i', $meta[2][$key][0], $tmp)) {
                    ...
                    '  if($array["content"] == false){ $content = soy2_h($array["insert"]) . $content . soy2_h($array["append"]); }else{ $content = soy2_h($array["content"]); }' .
                    '}' .
                    'echo \'<meta name="'.soy2_h($name).'" content="\'.$content.\'">\' . "\n"; ?>';
...
class HTMLTree extends SOYBodyComponentBase
{
    ...
    public function parseTree($tree, $depth = 0)
    {
        $innerHTML = $this->getInnerHTML();
        $list = array();
        $counter = 0;
    ...
    public function populateItemImpl($entity, $key, $depth, $isLast = false)
    {
        if (method_exists($this, "populateItem")) {
...
class PluginBase extends SOY2HTML
{
    ...
    public function setSoyValue($value)
    {
        $this->soyValue = $value;
    }
...
class SOY2HTML_ControllPlugin extends PluginBase
{
    public function getStartTag()
    {
        ...
        return '<?php $condition = ControllPlugin::checkCondition("' . $this->soyValue . '","' . soy2_h($condition) . '");' .
...
class SOY2Logger
{
   ...
   * ログを出力する
   */
    public function log($level, $levelText, $str)
    {
        if (!($this->level & $level)) {
            return;
        }
   ...
   */
    public function format($level, $str, $id, $logger)
    {
...
class SOY2Logger_RotationFileLogger extends SOY2Logger_FileLogger
{
    ...
    public function __construct($options)
    {
        if (isset($options["line"])) {
            $this->setMaxLineCount((int)$options["line"]);
        }
        if (isset($options["count"])) {
            $this->setMaxFileCount((int)$options["count"]);
        }
        if (isset($options["suffix"])) {
            $this->setSuffix((string)$options["suffix"]);
        }
        parent::__construct($options);
    }
  /**
   ...
   * ファイルのローテーションを実行する
   */
    public function rotation($filepath, $contents)
    {
...
{
    public function run($extetensionId, $moduleId, SOY2PluginAction $action);
}
/**
 * @package SOY2.SOY2Plugin
 */
...
class SOY2Plugin
{
        ...
        foreach ($extensions as $extensionId => $extensionArray) {
            foreach ($extensionArray as $moduleId => $array) {
                foreach ($array as $extensionClassName) {
                    ...
                    if (!($class instanceof SOY2PluginAction)) {
                        return;
                    }
   ...
   */
    ...
    public function setDelegate($point, $delegate)
    {
   ...
   */
    public function getDelegate($point)
    {
        if (!isset($this->delegates[$point])) {
            return false;
        }
        ...
        if (!($delegate instanceof SOY2PluginDelegateAction)) {
            return false;
        }
        return $delegate;
    }
   ...
   * 拡張ポイントに追加
   */
    public function addExtension($extension, $moduleId, $extensionClass)
    {
        if (!isset($this->extensions[$extension])) {
            $this->extensions[$extension] = array();
        }
        if (!isset($this->extensions[$extension][$moduleId])) {
            $this->extensions[$extension][$moduleId] = array();
        }
   ...
   */
    ...
    public function getExtensions($extensionId = null)
    {
        if (null!==$extensionId) {
...
class SOY2Session
{
   ...
   * get session
   */
    final public static function get($sessionClass)
    {
    ...
    public static function getSession($sessionClass)
    {
        ...
        if (!isset($_SESSION[self::_KEY_])) {
            $_SESSION[self::_KEY_] = array();
        }
...
function soy2_realurl($url)
{
  //末尾が拡張子の場合はそのまま
    ...
    if (!strlen($arg) || $arg == "_notfound") {
        return $url;
    }
    if (preg_match('/\.html$|\.htm$|\.xml$|\.css$|\.js$|\.json$|\.php$/i', $arg)) {
        return $url;
    }
    return $url . "/";
}
...
{
    if (null===$string || !is_string($string) || 1 > strlen($string)) {
        return null;
    } else {
        return unserialize(stripslashes($string));

//      $var = unserialize(stripslashes($string));
//      if( false === $var && "b:0;" !== $string ){
//          ob_start();
//          debug_print_backtrace();
//          $dpb = ob_get_clean();
//          error_log("DBG:unserialize failed. [" . $string . "] : " . __METHOD__ . "\n" . $dpb);
//      }
//      return $var;
    }
...
function soy2_get_token()
{
    ...
    if (!isset($_SESSION["s_token"])) {
        $_SESSION["s_token"] = soy2_generate_token();
    }
...
function soy2_check_token()
{
    ...
    if (isset($_SESSION["s_token"]) && isset($_REQUEST["s_token"])) {
        if ($_REQUEST["s_token"] === $_SESSION["s_token"]) {
            $_SESSION["s_token"] = soy2_generate_token();
            return true;
        }
    }
...
function soy2_check_referer()
{
    ...
    if ($referer['host'] . $port !== $_SERVER['HTTP_HOST']) {
        return false;
    }
  ...
  //pathinfoがある時は削除する
    ...
    if (isset($queryString) && strlen($queryString)) {
        $_path .= "?" . $queryString;
    }
...
{
    if (!count($opts)) {
        $opts = session_get_cookie_params();  //optsが空の場合はセッションの設定を用いる
    }
    if ((null===$value)) {
        $opts["expires"] = time()-1;  //valueがnullの場合はクッキーを削除する
    }
    if (isset($opts["lifetime"])) {
        unset($opts["lifetime"]);  //lifetimeがある場合は削除
    }

    if (!isset($opts["path"]) || !is_string($opts["path"]) || !strlen($opts["path"])) {
        $opts["path"] = "/";
    }
    if (!isset($opts["domain"]) || !is_string($opts["domain"]) || !strlen($opts["domain"])) {
        $opts["domain"] = null;
    }
    if (!isset($opts["expires"]) || !is_numeric($opts["expires"])) {
        $opts["expires"] = 0;
    }
    if (!isset($opts["httponly"]) || !is_bool($opts["httponly"])) {
        $opts["httponly"] = true;
    }
    if (!isset($opts["secure"]) || !is_bool($opts["secure"])) {
        $opts["secure"] = (isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] == "on");
    }
...
/* function/function.soy2_util.php */
function soy2_hash($data, $raw_output = false)
{
    return hash("sha256", $data, $raw_output);
}
function soy2_h($content, $charset = SOY2::CHARSET)
{
    return htmlspecialchars($content, ENT_QUOTES|ENT_SUBSTITUTE, $charset);
}
function soy2_escape_phptag($content)
{
