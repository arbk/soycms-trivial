...
class EntryTemplateDAO extends SOY2DAO
{
     ...
     * テンプレートファイルを書き出す
     */
    public function insert(EntryTemplate $temp, $filename = null)
    {
        if ((null===$filename)) {
            $max = 0;
        ...
        $res = '<?xml version="1.0" encoding="' . SOY2::CHARSET . '"?><template>' . implode("\n", $xml) . '</template>';

        $ret = file_put_contents($this->getTemplateDirectory()."/".$filename, $res);
        if ($ret) {
            @chmod($this->getTemplateDirectory()."/".$filename, F_MODE_FILE);
        }
        return $ret;
    }
     ...
     * テンプレートを更新する
     */
    public function update(EntryTemplate $template)
    {
      //内部的には現在の内容で上書きをする
