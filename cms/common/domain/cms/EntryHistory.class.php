...
class EntryHistory
{
    ...
    public function getChangeText()
    {
        $text = array();
        if ($this->changeTitle) {
            $text[] = CMSMessageManager::get("SOYCMS_ENTRY_TITLE");
        }
        if ($this->changeContent) {
            $text[] = CMSMessageManager::get("SOYCMS_ENTRY_CONTENT");
        }
        if ($this->changeMore) {
            $text[] = CMSMessageManager::get("SOYCMS_ENTRY_MORE");
        }
        if ($this->changeAdditional) {
            $text[] = CMSMessageManager::get("SOYCMS_ENTRY_ETC");
        }
        return implode(", ", $text);
    }
