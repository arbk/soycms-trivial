...
class CMSApplication
{
   ...
   */
    ...
    {
        $self = CMSApplication::getInstance();
        return CMSApplication::getRoot() . F_FRCTRLER . '/' . $self->applicationId . "/";
    }
   ...
   */
    ...
    {
        $self = CMSApplication::getInstance();
        if (!isset($self->properties["display_edit_panel"])) {
            return true;  //設定が特になければ出力する
        }
...
//     }
   ...
   */
    public static function printUpperMenu()
    {
        ...
        foreach ($menus as $key => $menu) {
            ...
            $href = (isset($menu["href"])) ? ' href="' . soy2_h($menu["href"]) . '" ' : ' href="javascript:void(0);" ';
            $onclick = (isset($menu["onclick"])) ? ' onclick="' . soy2_h($menu["onclick"]) . '" ' : "";
    ...
    public static function printTabs()
    {
      ...
      // modeプロパティにはどのテンプレートを使うか？の値が格納されている。modeプロパティはrun関数で定義されている
        ...
        foreach ($tabs as $key => $tab) {
            ...
            $href = (isset($tab["href"])) ? ' href="' . soy2_h($tab["href"]) . '" ' : ' href="javascript:void(0);" ';
            $onclick = (isset($tab["onclick"])) ? ' onclick="' . soy2_h($tab["onclick"]) . '" ' : "";
  ...
  //    require_once(CMS_COMMON . "config/custom.config.php");
  //  }else{
        require_once(CMS_COMMON . "soycms.config.php");
  //  }
      ...
      // アプリケーションのチェック
        ...
        if (!file_exists($base . $self->applicationId) || !file_exists($base . $self->applicationId . "/admin.php")) {
            SOY2PageController::redirect("../admin/");
            exit();
        }
      ...
      // ApplicationIdの登録
        if (!defined("APPLICATION_ID")) {
            define("APPLICATION_ID", $self->applicationId);
        }

      // 設定ファイルの読み込み
        $self->properties = (file_exists($base . $self->applicationId . "/application.ini"))
                      ? parse_ini_file($base . $self->applicationId . "/application.ini", false, INI_SCANNER_RAW)
                      : array();
    ...
    //      if( IS_EXT_MODE ){$self->mode = $properties["template"];}
    //    }
    //    else{
    ...
    //    }
      ...
      // アプリケーションの読み込み
        require_once($base . $self->applicationId . "/admin.php");

      // appMain設定されてるかな？
        if (null===$self->appMain) {
            SOY2PageController::redirect("../admin/");
            exit();
        }
    ...
    public static function display()
    {
        ...
        if (isset($_GET["mode"]) && $_GET["mode"] == "print") {
          ...
          //隠しモードロゴ usage：/cmsインストールディレクトリ/app/webapp/{アプリケーションID}/logo/以下に画像ファイルを一枚配置する
            $logoDir = dirname(__DIR__) . "/" . $self->applicationId . "/logo/";
            $fileName = null;
          ...
          //サイドバーを折りたたむか？
            ...
            require_once(__DIR__ . "/" . $self->mode . ".php");
        }
    }
   ...
   */
    public static function addScript($scriptUrl, $scriptBody = "")
    {
        ...
        if (strlen($scriptUrl) > 0) {
            $obj->scripts[] = '<script type="text/javascript" src="' . soy2_h($scriptUrl) . '"></script>';
        } elseif (strlen($scriptBody)) {
   ...
   */
    public static function addLink($cssUrl)
    {
        ...
        if (strlen($cssUrl) > 0) {
            $obj->links[] = '<link rel="stylesheet" href="' . soy2_h($cssUrl) . '">';
        }
    }
...
//          }
   ...
   */
    public static function switchAdminMode()
    {
      ...
      // DB
        require_once(CMS_COMMON . "config/db/" . SOYCMS_DB_TYPE . ".php");
        SOY2DAOConfig::Dsn(ADMIN_DB_DSN);
        SOY2DAOConfig::user(ADMIN_DB_USER);
        SOY2DAOConfig::pass(ADMIN_DB_PASS);
    }
   ...
   */
    public static function getAppAuthLevel()
    {
      ...
      // ディフォルトユーザの場合は無条件で SUPER USER にする
        if ($session->getAttribute("isdefault")) {
            return 1;  //SUPER USER
        }
