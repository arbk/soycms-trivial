...
class SOYInquiry_PageApplication
{
    ...
    public function getForm($formId)
    {
        ...
        define("SOYINQUERY_SOYSHOP_CONNECT_SITE_ID", null);
        ...
        if (!defined("SOYSHOP_INQUIRY_FORM_THEME")) {
            define("SOYSHOP_INQUIRY_FORM_THEME", $config->getTheme());
        }
        ...
        if (isset($_GET["block"])) {
            ...
            if (file_exists($templateDir . "ban.php")) {
                include($templateDir . "ban.php");
            } else {
                include(SOY2::RootDir() . "template/_sample/ban.php");
            }
        ...
        if (isset($_GET["captcha"])) {
            ...
            if (empty($errors)) {
                ...
                try {
                    ...
                    // とりあえずタイムアウトを30分にする！
                    if (SOYCMS_NOW - $inquiry->getCreateDate() > 30 * 60) {
                        throw new Exception();
                    }
                } catch (Exception $e) {
                ...
                ob_start();
                $this->outputCSS();
                include($templateDir . "complete.php");
                $html = ob_get_contents();
                ob_end_clean();
        ...
        } elseif (isset($_POST["form"]) || isset($_POST["form_x"])) {
            // フォームに戻る
            $errors = $this->checkPostData($_POST["data"], $columns);
        } elseif (isset($_POST["test"]) || isset($_POST["test_x"])) {
            // テスト：郵便番号検索を含む
            $errors = $this->checkPostData($_POST["data"], $columns);
        ...
        if (isset($_POST["confirm"]) || isset($_POST["confirm_x"])) {
            ...
            if (empty($errors)) {
                ...
                ob_start();
                $this->outputCSS();
                include($templateDir . "confirm.php");
                $html = ob_get_contents();
                ob_end_clean();
    ...
    public function checkBanMailAddress($data, $columns)
    {
        foreach ($columns as $column) {
            if (strpos($column->getType(), "MailAddress") !== false && $column->getRequire() == 1) {
                ...
                } elseif (isset($data[$column->getId()])) {
                    //上と同様
     ...
     * 問い合わせを追加する（メール送信を含む）
     */
    public function addInquiry($formId, $columns, $data, $url)
    {
        ...
        if ($res) {
            ...
            foreach ($columns as $column) {
                if ($column->getType() == "SerialNumber") {
                    $config = soy2_unserialize($column->getConfig());
                    if (!isset($config["serialNumber"]) || !is_numeric($config["serialNumber"])) {
            ...
            if (is_readable($this->templateDir . "mail.admin.php")) {
                ob_start();
                include($this->templateDir . "mail.admin.php");
            ...
            if (is_readable($this->templateDir . "mail.user.php")) {
                ob_start();
                include($this->templateDir . "mail.user.php");
            ...
            if ($logic->checkRecentInquiryCount($ipAddress)) {
                $logic->banIPAddress($ipAddress);
            }
        }
        return $inquiry;
    }
     ...
     * メールを送る
     */
    public function sendEmail($inquiry, $columns, $mailBody)
    {
        ...
        foreach ($columns as $column) {
            ...
            } elseif ($column->getType() == "ConfirmMailAddress" && is_array($column->getValue())) {
                $value = $column->getValue();
                $userMailAddr = (isset($value[0])) ? $value[0] : null;
                if (null===$userMailAddr) {
                    $userMailAddr = (isset($value[1])) ? $value[1] : null;
                }
                if (isset($userMailAddr)) {
                    $userMailAddress[] = $userMailAddr;
                }
     ...
     * Inquiryへのリンク
     */
    public function getInquiryLink($inquiry, $serverConfig)
    {
        $url = $serverConfig->getAdminUrl();
        if ($url[strlen($url) - 1] != "/") {
            $url .= "/";
        }
        if (!preg_match('/' . F_FRCTRLER . '$/', $url)) {
            $url .= F_FRCTRLER . "/";
        }
     ...
     */
    public function outputCSS($link = true)
    {
        if ($this->config->isOutputDesign() && file_exists($this->templateDir . "style.php") && method_exists($this->page, "getHeadElement")) {
            if ($link) {
                $this->page->getHeadElement()->insertHTML(
                    "<link rel=\"stylesheet\" title=\"inquiry\" type=\"text/css\" href=\"" . $this->pageUrl . "?stylesheet&amp;" . ((int)(SOYCMS_NOW / 100)) . "\">\n"
                );
            } else {
                $this->page->getHeadElement()->appendHTML(
