...
class IndexPage extends WebPage
{
    ...
    public function buildList($inquiries)
    {
      /* 問い合わせ */
      ...
      //問い合わせがないとき
        ...
        "colspan" => ( null===$this->formId and count($this->forms) >= 2 ) ? "6" : "5"
        ));

      //フォームが一つしかないときとフォームが指定されているときはフォーム名は表示しない
        $this->createAdd("form_name_th", "HTMLModel", array(
        "visible" => ( null===$this->formId and count($this->forms) >= 2 ),
        ));
        $this->createAdd("bulk_modify_buttons", "HTMLModel", array(
        "colspan" => ( null===$this->formId and count($this->forms) >= 2 ) ? "5" :"4"
        ));
    ...
    public function buildForm($start, $end, $trackId, $flag, $commentFlag = null)
    {
      /* 絞り込むフォームの作成 */
    ...
    public function buildPager(PagerLogic $pager)
    {
...
class InquiryList extends HTMLList
{
    ...
    protected function populateItem($entity)
    {
        ...
        "style"   => "cursor:pointer;". (($entity->getFlag() == SOYInquiry_Inquiry::FLAG_NEW) ? "color:black;font-weight: bold;" : ""),
            "visible" => (null===$this->formId && count($this->forms) >= 2),
