...
abstract class SOYInquiry_InquiryDAO extends SOY2DAO
{
     ...
     * @trigger onInsert
     */
    abstract public function insert(SOYInquiry_Inquiry $bean);

    abstract public function update(SOYInquiry_Inquiry $bean);
     ...
     * @columns id,flag
     */
    abstract public function updateFlagById($id, $flag);
     ...
     * @return object
     */
    abstract public function getByTrackingNumber($trackingNumber);
     ...
     */
    public function search($formId, $start, $end, $trackId, $flag, $commentFlag = null)
    {
        $query = $this->getQuery();
        $binds = $this->getBinds();
        ...
        foreach ($res as $row) {
            ...
            if ($commentFlag) {
                ...
                if ($commentFlag == SOYInquiry_Inquiry::COMMENT_HAS && !count($comments)) {
                    continue;
                }

                //コメント無し コメントがある場合はスルー
                if ($commentFlag == SOYInquiry_Inquiry::COMMENT_NONE && count($comments)) {
                    continue;
                }
            }
     ...
     */
    abstract public function countUnreadInquiryByFormId($formId);
     ...
     */
    abstract public function countInquiryByFormIdByFlag($formId, $flag);
     ...
     */
    abstract public function countUndeletedInquiryByFormId($formId);
