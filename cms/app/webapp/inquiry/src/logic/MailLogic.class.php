...
class MailLogic extends SOY2LogicBase
{
     ...
     * フォーム別の設定で送信元を上書きする
     */
    public function checkFormConfig(SOYInquiry_FormConfig $config)
    {
     ...
     */
    public function sendMail($sendTo, $title, $body, $sendToName = "", $replyTo = null, $replyToOnly = false)
    {
        //リセット
        $this->reset();
        ...
        $replyToArray = array();
        if ($replyTo) {
            $replyToArray = $replyTo;
        }
        if ($replyToOnly) {
          //何もしない
        } else {
            if ($this->replyTo) {
                $replyToArray[] = $this->replyTo->getString();
            }
        }
     ...
     *
     */
    public function sendNotifyMail($columns, $userMailAddress, $mailBody)
    {
            ...
            if (count($sendTo)) {
                ...
                foreach ($sendTo as $email) {
                    try {
                        ...
                        //管理者へ送信失敗
                        $eLogPath = CMS_COMMON."log/inquiry-mail-error-admin.log";
                        file_put_contents(
                            $eLogPath,
                        ...
                        @chmod($eLogPath, F_MODE_FILE);

                        $mLogDirPath = CMS_COMMON."log/inquiry";
                        if (!file_exists($mLogDirPath)) {
                            mkdir($mLogDirPath);
                            @chmod($mLogDirPath, F_MODE_DIR);
                        }
                        $mLogPath = $mLogDirPath."/mail-".date("YmdHi-").($columns[0]->getInquiry()->getTrackingNumber())."-admin.log";
                        file_put_contents($mLogPath, var_export($e, true)."\n\n", FILE_APPEND);
                        @chmod($mLogPath, F_MODE_FILE);
                    }
                }
            }
        }
        ...
        if ($formConfig->getIsSendConfirmMail()) {
            if (count($userMailAddress)>0) {
                ...
                $content = array();
                if (strlen($array["header"])>0) {
                    $content[] = $this->replaceText($columns, $array["header"]);
                }
                if ($array["isOutputContent"]>0) {
                    $content[] = htmlspecialchars_decode($mailBody[1], ENT_QUOTES);
                }
                if (strlen($array["footer"])>0) {
                    $content[] = $this->replaceText($columns, $array["footer"]);
                }
                $content = implode("\r\n", $content);
                foreach ($userMailAddress as $email) {
                    try {
                        ...
                        //ユーザへ送信失敗
                        $eLogPath = CMS_COMMON."log/inquiry-mail-error-user.log";
                        file_put_contents(
                            $eLogPath,
                            date("r")." ".$columns[0]->getInquiry()->getTrackingNumber()."\n".
                            "From: ".$this->send->getFrom()->getAddress()."\n".
                        ...
                        $mLogPath = $mLogDirPath."/mail-".date("YmdHi-").($columns[0]->getInquiry()->getTrackingNumber())."-user.log";
                        file_put_contents($mLogPath, var_export($e, true)."\n\n", FILE_APPEND);
                        @chmod($mLogPath, F_MODE_FILE);
                    }
                }
    ...
    private function replaceText($columns, $text)
    {
        ...
        foreach ($columns as $column) {
            ...
            if (strlen($replace)>0 && strpos($text, $replace) !== false) {
                if (strlen($replace)>0 && strpos($text, $replace) !== false) {
                    $text = str_replace($replace, htmlspecialchars_decode($obj->getMailText(), ENT_QUOTES), $text);
                }
            }
        }
        return $text;
