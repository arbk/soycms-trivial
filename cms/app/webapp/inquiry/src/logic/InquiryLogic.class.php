...
class InquiryLogic extends SOY2LogicBase
{
     ...
     * カラム→問い合わせ本文作成
     */
    public function getInquiryBody($inquiry, $columns, $useMailBody = false)
    {
        $body = array();
     ...
     * 問い合わせメール本文作成
     */
    public function getInquiryMailBody($inquiry, $columns)
    {
     ...
     */
    public function addInquiry($formId)
    {
        ...
        $inquiry->setCreateDate(SOYCMS_NOW);
     ...
     * 問い合わせ情報を更新
     */
    public function updateInquiry(SOYInquiry_Inquiry $inquiry, $body, $data, $url)
    {
        $inquiryDao = $this->getDAO();
        ...
        //問い合わせ番号生成->保存
        if (null===$inquiry->getTrackingNumber()) {
     ...
     * @return number
     */
    public function countUnreadInquiryByFormId($formId)
    {
     ...
     * @return number
     */
    public function countInquiryByFormIdByFlag($formId, $flag)
    {
     ...
     * @return number
     */
    public function countUndeletedInquiryByFormId($formId)
    {
    ...
    {
        if (!$this->dao) {
            $this->dao = SOY2DAOFactory::create("SOYInquiry_InquiryDAO");
        }
        return $this->dao;
    }
     ...
     * onSend
     */
    public function invokeOnSend($inquiry, $columns)
    {
        ...
        $this->_updateToSOYMail($columns);
//      $this->_updateToSOYShop($columns);
    }
     ...
     * @return array
     */
    public function buildCSVData($inquiry, $columns)
    {
        $res = array();
    ...
    {
        list($sql, $binds) = $this->_buildQueryAndBinds($columns, "mail");
        if (!strlen($sql)) {
            return;
        }
    ...
    // {
    //     list($sql, $binds) = $this->_buildQueryAndBinds($columns, "shop");
    //     if (!strlen($sql)) {
    //         return;
    //     }
    ...
    //                 require_once(SOY2::RootDir() . "base/func/common.php");
    ...
    private function _buildQueryAndBinds($columns, $mode = "mail")
    {
        ...
        foreach ($values as $key => $val) {
            ...
            if ($key === "mail_address" && strpos($val, "@") > 0) {
                $isMailAddress = true;
            }
     ...
     * [3桁以上FormId]-
     */
    public function getTrackingNumber($inquiry)
    {
        for ($i = 0;; ++$i) {
     ...
     * 問い合わせのフラグを一括更新
     */
    public function bulk_update_flag($ids, $flag)
    {
        switch ($flag) {
    ...
    /** カートをIPアドレスで制限 **/
    public function checkRecentInquiryCount($ipAddress)
    {
    ...
    public function banIPAddress($ipAddress)
    {
    ...
    public function checkBanIpAddress()
    {
