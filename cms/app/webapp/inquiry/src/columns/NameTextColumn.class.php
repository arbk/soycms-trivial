...
class NameTextColumn extends SOYInquiry_ColumnBase
{
   ...
   */
    public function getForm($attr = array())
    {
      ...
      //入力モード指定
        ...
        $html[] = $this->label1;
        $html[] = "<input type=\"text\" name=\"data[".$this->getColumnId()."][0]\" value=\"".soy2_h($values[0])."\" " . implode(" ", $attributes) . " ".$this->getFormAttribute($this->attribute1) . $required . ">&nbsp;";

        $html[] = $this->label2;
        $html[] = "<input type=\"text\" name=\"data[".$this->getColumnId()."][1]\" value=\"".soy2_h($values[1])."\" " . implode(" ", $attributes) . " ".$this->getFormAttribute($this->attribute2) . $required . ">";
   ...
   */
    ...
    {
        $values = $this->getValue();
        return soy2_h($values[0]." ".$values[1]);
    }
