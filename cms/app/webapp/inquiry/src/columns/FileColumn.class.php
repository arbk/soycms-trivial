...
class FileColumn extends SOYInquiry_ColumnBase
{
   ...
   */
    public function getForm($attr = array())
    {
      ...
      //アップロードされていた場合
        if ($isUploaded) {
            $html[] = soy2_h($values["name"]) . "(".(int)($values["size"] / self::KB_SIZE)."KB)";
   ...
   */
    public function getConfigForm()
    {
        ...
        $html .= 'height:<input type="text" name="Column[config][resize_h]" value="'.soy2_h($this->resize_h) . '" size="5">px(アスペクト比は維持)';
        ...
        if (is_array($values)) {
            ...
            if (rename($tmp_name, $new_name)) {
              ...
              //コメントに追加する
                ...
                $content .= '<a href="'.soy2_h($values["filepath"]).'">'.soy2_h($values["name"]).'</a>';
                ...
                if (count($extensions)) {
                    ...
                    if ($res) {
                        $content .= '<br><img src="'.soy2_h($values["filepath"]).'">';
                    }
                }
    ...
    public function validate()
    {
      ...
      //一時的にアップロードする
        if (!file_exists(SOY_INQUIRY_UPLOAD_TEMP_DIR)) {
            ...
            @chmod(SOY_INQUIRY_UPLOAD_TEMP_DIR, F_MODE_DIR);
        }
