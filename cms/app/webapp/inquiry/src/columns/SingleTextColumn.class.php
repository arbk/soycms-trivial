...
class SingleTextColumn extends SOYInquiry_ColumnBase
{
    ...
    public function getForm($attr = array())
    {
        ...
        $html = array();
        $html[] = "<input type=\"" . soy2_h($this->inputType) . "\" name=\"data[".$this->getColumnId()."]\" value=\"".soy2_h($this->getValue())."\" " . implode(" ", $attributes) . $required . ">";
    ...
    public function getConfigForm()
    {
        ...
        $html .= '<input  id="Column[config][prefix]'.$this->getColumnId().'" name="Column[config][prefix]" type="text" value="'.soy2_h($this->prefix).'" size="10">&nbsp;';
        $html .= '<label for="Column[config][surfix]'.$this->getColumnId().'">接尾辞:</label>';
        $html .= '<input  id="Column[config][surfix]'.$this->getColumnId().'" name="Column[config][surfix]" type="text" value="'.soy2_h($this->surfix).'" size="10">&nbsp;';
        ...
        $inputType = (isset($this->inputType) && strlen($this->inputType) > 0) ? soy2_h($this->inputType) : "text";
...
//    return soy2_h((string)$value);
//  }

    /**
     * 絵文字削除
     */
    public function deleteEmoji($value)
    {
    ...
    public function getContent()
    {
        ...
        if (strlen($content)) {
            return soy2_h($this->prefix).$content.soy2_h($this->surfix);
        } else {
            return "";
        }
    }
