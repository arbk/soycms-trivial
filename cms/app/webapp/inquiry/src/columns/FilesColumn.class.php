...
class FilesColumn extends SOYInquiry_ColumnBase
{
        ...
        if (count($values)) {
            foreach ($values as $v) {
                if (isset($v["tmp_name"]) && strlen($v["tmp_name"])) {
                    $html[] = soy2_h($v["name"]) . "(".(int)($v["size"] / self::KB_SIZE)."KB)";
                    $html[] = "<br>";
                }
            }
   ...
   */
    ...
    {
        $html  = 'サイズ:<input type="text" name="Column[config][uploadsize]" value="'.soy2_h($this->uploadsize).'" size="3">KB&nbsp;';
        $html .= '拡張子:<input type="text" name="Column[config][extensions]" value="'.soy2_h($this->extensions).'"><br>';
        $html .= '画像のリサイズ: width:<input type="text" name="Column[config][resize_w]" value="'.soy2_h($this->resize_w) . '" size="5">px ';
        $html .= 'height:<input type="text" name="Column[config][resize_h]" value="'.soy2_h($this->resize_h) . '" size="5">px(アスペクト比は維持)<br>';
        ...
        if (is_array($values) && count($values)) {
            ...
            foreach ($values as $v) {
                if (isset($v["tmp_name"]) && strlen($v["tmp_name"])) {
                    ...
                    $html[] = "<span style=\"color:red;\">" . soy2_h($v["name"]) . "のアップロードを失敗しました。</span>";
                }
            }
      ...
      //アップロードした
        ...
        if (isset($_FILES["data"]["size"]) && is_array($_FILES["data"]["size"][$id]) && count($_FILES["data"]["size"][$id]) && (int)$_FILES["data"]["size"][$id][0] > 0) {
            ...
            for ($i = 0; $i < count($_FILES["data"]["size"][$id]); $i++) {
                if (!isset($_FILES["data"]["size"][$id][$i]) || (int)$_FILES["data"]["size"][$id][$i] === 0) {
                    continue;
                }
                $v = array();
                ...
                $extensions = $this->_shapeExtensions();
                if (count($extensions)) {
                    $res = false;
                    foreach ($extensions as $ext) {   //大文字小文字関係なく拡張子を確かめる
                        if (!$res && is_numeric(stripos($pathinfo["extension"], $ext))) {
                            $res = true;
                        }
                    }
   ...
   * onSend
   */
    public function onSend($inquiry)
    {
        $values = $this->getValues();
        if (is_array($values) && count($values)) {
            ...
            foreach ($values as $i => $v) {
              ...
              //コメントに追加する
                    ...
                    $content .= '<a href="' . soy2_h($v["filepath"]) . '">' . soy2_h($v["name"]) . '</a>';
                    ...
                    if (count($extensions)) {
                        ...
                        if ($res) {
                            $content .= '<br/><img src="' . soy2_h($v["filepath"]) . '"/>';
                        }
                    }
    ...
    private function getValues()
    {
        ...
        if (is_string($values) && strlen($values)) {
            $values = soy2_unserialize(base64_decode($values));
        }
   ...
   */
    ...
    public function getConfigure()
    {
      ...
      //片方しか値を登録していない場合はwidthとheightで補完し合う。
        if (null===$config["resize_w"] && is_numeric($config["resize_h"])) {
            $config["resize_w"] = $config["resize_h"];
        }
        if (null===$config["resize_h"] && is_numeric($config["resize_w"])) {
            $config["resize_h"] = $config["resize_w"];
        }
        return $config;
    }
