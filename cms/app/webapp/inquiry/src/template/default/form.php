  ...
  //連番カラムは表示しない
    ...
    if (strlen($label)>0) {
        ...
        if (($column->getType() == "Enquete" || $column->getType() == "EnqueteFree")) {  //アンケートカラムの場合は文言を大きく表示
            ...
            $label = (isset($config["question"]) && strlen($config["question"])) ? soy2_h($config["question"]) : $obj->getLabel();
            $output .= "<th colspan=\"2\">\n";
            $output .= $label;
            $output .= "\n</th>\n";
            $output .= "</tr><tr>";
...
//最初のプライバシーポリシー
if (isset($beforeForm)) {
    echo '<table class="inquiry_form" id="inquiry_privacy_policy">'.$beforeForm.'</table>';
}
//フォーム本体
...
//最後のプライバシーポリシー
if (isset($afterForm)) {
    echo '<table class="inquiry_form" id="inquiry_privacy_policy">'.$afterForm.'</table>';
}
